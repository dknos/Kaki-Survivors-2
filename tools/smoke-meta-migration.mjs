...
//
// Run: node --experimental-loader ./tools/_loader.mjs tools/smoke-meta-migration.mjs
//
...
async function scenario3V2Roundtrip() {
  ...
  assert.equal(d.unlockedKakiLand, false,            'v2: missing Kaki flag backfills safely');
  assert.equal(d.optRenderer, 'webgl',                'v2: valid renderer preference preserved');
...
async function scenario4Api() {
  ...
  // Renderer settings are additive save metadata but strictly whitelisted.
  m.setOption('optRenderer', 'webgpu');
  assert.equal(m.getMeta().optRenderer, 'webgpu',     'api: valid renderer option sticks');
  assert.equal(JSON.parse(ls.getItem('kk-survivors-meta-v2')).optRenderer, 'webgpu');
  m.setOption('optRenderer', 'webgl2');
  assert.equal(m.getMeta().optRenderer, 'auto',       'api: invalid renderer option normalizes');
  m.getMeta().optRenderer = 'corrupt-direct-mutation';
  assert.equal(m.saveMeta(), true);
  assert.equal(m.getMeta().optRenderer, 'auto',       'api: saveMeta normalizes legacy direct mutation');
  assert.equal(JSON.parse(ls.getItem('kk-survivors-meta-v2')).optRenderer, 'auto');
