 ...
 *   dpsBuckets   : state.run._dpsWin.buckets.length  (fixed at 50)
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < SAMPLE_COUNT; i++) {
      ...
      const sample = await page.evaluate(() => {
        ...
        return {
          ...
          gems: gems.length,
          webs: webs.length,
          dpsBuckets: (dpsWin && dpsWin.buckets) ? dpsWin.buckets.length : 0,
    ...
    lines.push('');
    lines.push('| #  | wall s | game s |   heap MB | enemies | weapons | proj | gems | webs | dpsBuckets | sealed | ach | toasts | DOM   |');
    lines.push('|----|-------:|-------:|----------:|--------:|--------:|-----:|-----:|-----:|-------:|-------:|----:|-------:|------:|');
    for (let i = 0; i < samples.length; i++) {
      ...
      lines.push(
        ...
        + ' | ' + String(s.dpsBuckets).padStart(10, ' ')
    ...
    lines.push('');
    lines.push('- `state.run._dpsWin` is a fixed 50×100ms bucket ring advanced by');
    lines.push('  `src/dpsWindow.js`. `dpsBuckets` must remain exactly 50. **Not a leak.**');
...
function _leakHint(metric) {
  switch (metric) {
    ...
    case 'dpsBuckets':
      return 'state.run._dpsWin is fixed at 50 buckets; any count change means its ring contract regressed.';
    case 'pendingVolat':
