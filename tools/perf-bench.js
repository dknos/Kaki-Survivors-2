 ...
 *
 * Run: node tools/perf-bench.js [spawnMul] [seconds] [avatar] [port]
 */
...
const PORT = Number(process.argv[5] || process.env.PORT || 8765);
const SPAWN_MUL = Number(process.argv[2] || 2.0);
const SECONDS = Number(process.argv[3] || 45);
const PERF_AVATAR = String(process.argv[4] || process.env.PERF_AVATAR || '').trim();
...
async function main() {
  ...
  await page.goto('http://127.0.0.1:' + PORT + '/index.html?perf=1', { waitUntil: 'load', timeout: 30000 });
  ...
  await page.evaluate(async ({ mul, avatar }) => {
    localStorage.setItem('kks_introSeen', '1');
    localStorage.setItem('kks_forestTrialsIntroSeen_v1', '1');
    const meta = await import('./src/meta.js');
    if (meta.setOption) meta.setOption('selectedStage', 'forest');
    if (avatar) {
      if (meta.unlockAvatar) meta.unlockAvatar(avatar, 'perf-bench');
      if (meta.setOption) meta.setOption('selectedAvatar', avatar);
    }
    await window.kkStartRun();
    const s = window.kkState;
    if (s && s.run) {
      // Exercise the public weekly multiplier path once. Setting both the
      // daily and weekly fields silently squares the requested stress value.
      s.run.weeklySpawnMul = mul;
      s.run.dailySpawnMul = 1;
      s.hero.hpMax = 1e9;
      s.hero.hp = 1e9;
    }
  }, { mul: SPAWN_MUL, avatar: PERF_AVATAR });
  await page.waitForFunction(
    () => !!window.kkState?.run && window.kkState.started === true
  ...
  const sceneInventory = await page.evaluate(() => {
    ...
    for (const root of s.scene.children) {
      let meshes = 0, instanced = 0, bloom = 0, materialSlots = 0;
      const roomMeshes = {};
      const meshRows = [];
      const walk = (o, parentVisible) => {
        const visible = parentVisible && o.visible !== false;
        if (visible && o.isMesh) {
          meshes++;
          if (o.isInstancedMesh) instanced++;
          if ((o.layers.mask & 2) !== 0) bloom++;
          materialSlots += Array.isArray(o.material) ? o.material.length : 1;
          const slots = Array.isArray(o.material) ? o.material.length : 1;
          if (root.name === '__arenaDecor') meshRows.push({
            name: o.name || o.userData?.kkForestAccent || o.userData?.landscapePurpose || o.type,
            slots,
            instances: o.isInstancedMesh ? o.count : 1,
            room: o.userData?.roomId || null,
          });
          const room = o.userData?.roomId;
          if (room) roomMeshes[room] = (roomMeshes[room] || 0) + 1;
        }
      ...
      if (meshes) rows.push({
        ...
        meshes, instanced, bloom, materialSlots, roomMeshes,
        heaviestMeshes: meshRows.sort((a, b) => b.slots - a.slots).slice(0, 30),
      });
    }
    return rows.sort((a, b) => b.materialSlots - a.materialSlots);
  });
  console.log('[bench] visible scene inventory');
  console.log(JSON.stringify(sceneInventory, null, 2));
  // Keep shader compilation/stage construction out of the steady-state window.
  await page.waitForTimeout(1500);
  await page.evaluate(() => window.kkPerfForceOn());
  ...
  if (valid.length === 0) {
    throw new Error('No valid FPS samples were captured after the run started');
  }
