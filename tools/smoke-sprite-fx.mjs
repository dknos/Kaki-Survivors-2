...
// runtime import of src/sprites/* — the browser-only node material graph and
// InstancedBufferAttribute setup are covered by renderer tests instead). Atlas
// validation is JSON-schema only.
//
...
check(`spritePool references backend-neutral BLOOM_LAYER`, () => {
  assert.ok(/\bBLOOM_LAYER\b/.test(poolSrc), 'BLOOM_LAYER not referenced');
  assert.ok(/from\s+['"]\.\.\/rendering\/bloomLayers(?:\.js)?['"]/.test(poolSrc),
...
check(`sprite material uses MeshBasicNodeMaterial`, () => {
  assert.ok(/new\s+MeshBasicNodeMaterial\s*\(/.test(materialSrc),
    'MeshBasicNodeMaterial is not instantiated');
});
check(`sprite material binds TSL instance attributes and texture sampling`, () => {
  for (const token of ["attribute('aFrame'", "attribute('aScale'", "attribute('aFlash'", 'texture(atlas.texture']) {
    assert.ok(materialSrc.includes(token), `missing TSL node expression: ${token}`);
  }
  assert.ok(/material\.vertexNode\s*=/.test(materialSrc), 'vertexNode is not assigned');
  assert.ok(/material\.outputNode\s*=/.test(materialSrc), 'outputNode is not assigned');
});
