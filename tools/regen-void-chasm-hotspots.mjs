// Build-time generator for the content-versioned Void hotspot roster.
//
...
import { isPointOnTerrainBridge } from '../src/stageTerrainLayout.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const OUT_PATH = resolve(__dirname, '..', 'assets', 'void_chasm_hotspots_20260710.json');
...
function deriveChasms(rand) {
  ...
  while (out.length < TARGET && guard++ < GUARD) {
    ...
    // The authored abyss fracture is dangerous, but its bridge decks are the
    // explicit safe crossings. Never stamp a circular chasm on top of one.
    if (isPointOnTerrainBridge('void', x, z, zRadius + 0.8)) continue;
