 ...
 *   2. entering Mossroot through its Glade portal starts the canonical
 *      three-wave trial, state.enemies.active contains tagged trial enemies,
 ...
 *   Phase 2 — activate Mossroot's real outbound Glade portal through the
 *             production interact edge. The portal-owned transfer token is
 *             consumed by _tickForestRoomTransition on the next frame, which
 *             starts wave 1/3 and seals the return portal. Direct position
 *             mutation is deliberately not used: normal Forest runs contain
 *             the hero to the current room unless a portal owns the transfer.
...
async function main() {
  ...
  // Gameplay validation must not depend on an external font CDN being
  // reachable from a headless/CI network namespace.
  ...
  page.on('console', (msg) => {
    ...
    const ty = msg.type();
    if (ty === 'error') {
      const externalResourceNoise = /Failed to load resource: net::ERR_(?:TIMED_OUT|NAME_NOT_RESOLVED|INTERNET_DISCONNECTED|BLOCKED_BY_CLIENT)/.test(t);
      if (!externalResourceNoise) {
  ...
  try {
    ...
    await page.evaluate(async () => {
      // This smoke validates gameplay, not the once-per-install story overlay.
      // Suppress it so phase screenshots and room probes see the live scene.
    ...
    // Wait for the async run transition to finish, not merely for kkState to
    // exist (it is published during boot, before the loading overlay closes).
    await page.waitForFunction(
      () => !!window.kkState && !!window.kkState.run
        && window.kkState.started === true && window.kkState.mode === 'run',
      null,
      { timeout: BOOT_TIMEOUT_MS },
    );

    // God-mode the AFK hero. This smoke asserts FOREST systems (rooms, fog,
    // reaper HUD) — not survival. Fun-loop iter 1.1 made standing still
    // lethal (by design), so an unprotected hero dies at the phase-4 time
    // jump (t=1780 → D=10 wave) and gameOver gates the tick before the
    // reaper banner ever paints.
    await page.evaluate(() => {
      const s = window.kkState;
      if (s && s.hero) { s.hero.hpMax = 1e9; s.hero.hp = 1e9; }
    });
    console.log('[smoke-v2] kkState live (hero god-moded); settling ' + PHASE_SETTLE_MS + 'ms for phase 1');
    ...
    // ── PHASE 2 — sealed Mossroot portal trial ────────────────────────────
    // Enter through the production portal interaction. This proves room
    // containment cannot be bypassed and that the portal-owned travel token
    // starts the new multi-wave trial.
    const portalActivated = await page.evaluate(async () => {
      const s = window.kkState;
      const mod = await import('/src/forestPortals.js');
      const portal = (mod.getForestPortals && mod.getForestPortals() || [])
        .find((p) => p && p.kind === 'outbound' && p.destRoomId === 'mossroot');
      if (!portal || !s || !s.hero) return false;
      ...
      mod.tickForestPortals(0, s);
      s.input.interactPressed = true;
      mod.tickForestPortals(1 / 60, s);
      s.input.interactPressed = false;
      return Math.hypot(s.hero.pos.x - portal.dest.x, s.hero.pos.z - portal.dest.z) < 0.1;
    });

    // Poll for room transition + wave-1 spawn — up to ROOM_SETTLE_MS.
    const roomReady = await pollUntil(page, () => {
      ...
      const room = trial && trial.rooms && trial.rooms.mossroot;
      const mobs = s.enemies.active.filter((e) => e && e.alive && e._forestTrialRoom === 'mossroot');
      return s.run.forestTrialActive === true && room && room.status === 'ACTIVE'
        && room.wave === 1 && mobs.length >= 1;
    }, ROOM_SETTLE_MS, 100);
    ...
    const p2 = await page.evaluate(async () => {
      ...
      const trial = s.run.forestPortalTrials;
      const trialRoom = trial && trial.rooms && trial.rooms.mossroot;
      const trialMobs = (s.enemies && s.enemies.active || [])
        .filter((e) => e && e.alive && e._forestTrialRoom === 'mossroot');
      const trialMobCount = trialMobs.length;
      if (!trialRoom || trialRoom.status !== 'ACTIVE' || trialRoom.wave !== 1 || trialMobCount < 1) {
        return {
          ok: false,
          reason: 'Mossroot wave 1 inactive (currentRoom=' + room + ', trial=' + JSON.stringify(trialRoom)
            + ', mobs=' + trialMobCount + ')',
          trialMobCount,
          room,
        };
      ...
      if (probeErr) {
        return { ok: false, reason: 'portal probe failed: ' + probeErr, trialMobCount, room };
      }
      ...
      const promptVisible = promptEl
        ? Number.parseFloat(getComputedStyle(promptEl).opacity || '0') > 0.1
        : false;
      return {
        ok: sealedCount >= 1,
        reason: sealedCount >= 1
          ? 'room=' + room + ', wave=' + trialRoom.wave + '/3, trialMobs=' + trialMobCount + ', sealedPortals='
            + sealedCount + '/' + portalCount
            + ', sealedPrompt=' + (promptEl ? 'present(visible=' + promptVisible + ')' : 'absent')
          : 'no sealed portals found (trialMobs=' + trialMobCount + ', room=' + room
            + ', totalPortals=' + portalCount + ')',
        trialMobCount, room, sealedCount, portalCount,
      };
    });
    let p2Pass = portalActivated && p2.ok && roomReady;
    let p2Reason = p2.reason
      + (portalActivated ? '' : ' [portal activation FAILED]')
      + (roomReady ? '' : ' [roomReady poll TIMEOUT]');
    if (p2Pass && fps2 <= FPS_LIVENESS_FLOOR) {
      p2Pass = false;
      p2Reason += '; rAF DEAD (fps=' + fps2.toFixed(2) + ')';
    }
    recordPhase(2, 'sealedroom', p2Pass, p2Reason, fps2,
      { trialMobCount: p2.trialMobCount, sealedCount: p2.sealedCount, room: p2.room });
