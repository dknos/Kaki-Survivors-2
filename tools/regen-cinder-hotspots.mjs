...
//   - facing = atan2(z, -x) for this engine's (cos(yaw), -sin(yaw)) forward
...
function verify(hotspots, catapults) {
  ...
  for (let i = 0; i < hotspots.length; i++) {
    ...
    if (typeof h.facing !== 'number' || !Number.isFinite(h.facing)) {
      ...
      const rInv = 1 / Math.max(0.001, Math.hypot(h.x, h.z));
      const forwardX = Math.cos(h.facing);
      const forwardZ = -Math.sin(h.facing);
      const towardCenter = forwardX * (-h.x * rInv) + forwardZ * (-h.z * rInv);
      if (towardCenter < 0.999) {
        badFacing++;
        console.warn(`  outward facing seed=${h.seed}: dot=${towardCenter.toFixed(4)}`);
      }
    }
  }
