 ...
 *   slot 6 #f5a300 — restrained idle emissive (0.22 ↔ 0.38 @ 0.7Hz)
 *   slot 7 #ffd86b — detonation flash emissive (single-frame peak 1.6)
...
const IDLE_EMISSIVE_MIN = 0.22;      // preserve authored facets in daylight
const IDLE_EMISSIVE_MAX = 0.38;
const DET_FLASH_EMISSIVE = 1.6;      // readable flash without white clipping
...
// Bake the same authored moonroot GLB used by arenaDecor into one shared amber
// geometry. Each interactive node retains its independent material/pulse while
// the placeholder cylinder-and-cones silhouette is gone everywhere in Forest.
function _buildAmberGeometry() {
  if (!GLTF_CACHE.forest_moonroot_crystal) {
    throw new Error('forest_moonroot_crystal was not preloaded');
  }
  ...
  src.traverse((o) => {
    ...
    const crown = String(o.name || '').toLowerCase().includes('crystal');
    const tint = new THREE.Color(crown ? COLOR_FACET : COLOR_BODY);
  ...
  if (!merged) {
    ...
    throw new Error('moonroot crystal base/crown geometry could not be merged');
  }
  if (parts.length > 1) parts.forEach((g) => g.dispose());
  merged.computeBoundingBox();
  merged.computeBoundingSphere();
  merged.userData.footY = merged.boundingBox.min.y;
  merged.userData.asset = 'moonroot_crystal_cluster.glb';
  return merged;
}
...
export async function loadForestAmber(scene, hotspotsUrl = 'assets/forest_amber_hotspots.json') {
  ...
  for (const h of hotspots) {
    ...
    const s = h.scale || 1;
    const footY = Number(sharedGeo.userData.footY) || 0;
    mesh.position.set(h.x, -footY * s + 0.01, h.z);   // rooted foot sits on floor
    ...
    mesh.userData.gameplayPurpose = 'shoot-to-detonate-amber-node';
    mesh.userData.bloomPolicy = 'silhouette-first';
    _group.add(mesh);
