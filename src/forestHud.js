 ...
 *
 * Compact Forest companion for the shared bottom command deck. Adds legibility
 * for the 30-minute Reaper arc without duplicating permanent run telemetry:
 *
 *   • Stage clock (accessible mirror)  — MM:SS reading state.time.game. The
 *                                        visible clock is ui.js's TIME metric;
 *                                        this node preserves stage lifecycle
 *                                        and assistive-technology output.
 *   • Reaper countdown (above deck)    — "REAPER IN M:SS" once gameT >=
 *                                        1680 (T-2:00 from 1800s spawn);
 *                                        flips to "REAPER ACTIVE" at 1800s.
 *
 * NB (2026-07-08): the kill + chest counters this module used to draw were
 * removed — the survivors HUD (ui.js .kk-stats) already renders KILLS, so
 * forest was showing it twice, and the chest counter was a dead "—"
 * placeholder never wired to a per-run source. As of 2026-07-18 the shared
 * command deck is canonical for clock, level, kills, and DPS, so Forest adds
 * only the late-run Reaper warning visually.
 *
...
function _ensureStyle() {
  ...
  s.textContent = [
    ...
    // The command deck's TIME metric is now the one visible run clock. Keep
    // this stage-tinted clock in the accessibility tree and lifecycle tests
    // without spending another top-center panel on duplicate information.
    '#' + CLOCK_ID + ' {',
    ...
    '}',
    '#' + REAPER_ID + ' {',
    '  position: absolute; top: auto; bottom: 96px; left: 50%;',
    '  transform: translateX(-50%);',
    '  padding: 5px 11px; background: rgba(30,15,17,.82);',
    '  border: 1px solid rgba(232,156,74,.46); border-radius: 999px;',
    '  box-shadow: 0 8px 18px rgba(12,6,8,.34);',
    ...
    '  #' + REAPER_ID + ' { bottom: 82px; padding: 4px 8px; font-size: 8px; }',
    '}',
