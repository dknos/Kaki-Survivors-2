 ...
 *
 * ── Pre-pool ───────────────────────────────────────────────────────────────
 * CAP = 4 active pickups. Pre-allocated InstancedMesh for the authored
 * cat-paw relic cutout (0.95m flat plane) + shared paw marker. Zero
 ...
 *
 * ── Palette ────────────────────────────────────────────────────────────────
 *   authored cocoa/gold/cyan relic art — untinted for identity
 *   shared pastel paw marker — interaction accent, no ring silhouette
 *
 * ── Self-gating ────────────────────────────────────────────────────────────
...
const BODY_Y           = 0.14;       // medallion sits above ground clutter
const BODY_SIZE        = 0.95;       // readable at normal camera zoom
...
let _relicTexDirect = null;
const _relicLoader = new THREE.TextureLoader();

function _weaponRelicTexture() {
  const authored = fxTex('pickup_weapon_relic');
  if (authored) return authored;
  if (!_relicTexDirect) {
    _relicTexDirect = _relicLoader.load(
      new URL('../assets/fx/pickups/weapon_relic_drop.webp', import.meta.url).href,
    );
    _relicTexDirect.colorSpace = THREE.SRGBColorSpace;
    _relicTexDirect.minFilter = THREE.LinearMipmapLinearFilter;
    _relicTexDirect.magFilter = THREE.LinearFilter;
  }
  return _relicTexDirect;
}
...
function _buildBodyMesh() {
  ...
  // reads the plane as a billboard — same trick used by forestPickup markers.
  const geo = new THREE.PlaneGeometry(BODY_SIZE, BODY_SIZE);
  geo.rotateX(-Math.PI / 2);
  const mat = new THREE.MeshBasicMaterial({
    map: _weaponRelicTexture(),
    color: 0xffffff,
    transparent: true,
    opacity: 1,
    alphaTest: 0.08,
  ...
  _bodyMesh.userData.asset = 'pickup_weapon_relic';
  _bodyMesh.userData.purpose = 'interactive weapon pickup';
...
function _buildSparkleMesh() {
  const marker = createPickupMarker({
    ...
    instanced: true,
    cap: CAP_PICKUPS,
    gameplayPurpose: 'walk-over weapon reward',
    userData: { pickupPart: 'weaponDropMarker' },
  });
  _sparkleMesh = marker.mesh;
  _track(marker.geometry); _track(marker.material);
}
...
function _stampSparkle(i, x, z, scale) {
  // A slow breathing paw badge gives the relic a readable invitation without
  // reverting to the fast spinning gold O that looked like an AoE marker.
  const yaw = i * 0.36 + Math.sin(_clock * 1.15 + i) * 0.08;
  const bob = SPARKLE_Y + Math.sin(_clock * 2.2 + i * 1.2) * 0.06;
  _dummy.position.set(x, bob, z);
  _dummy.rotation.set(0, yaw, 0);
  _dummy.scale.setScalar(scale * (1 + Math.sin(_clock * 2.4 + i) * 0.045));
  _dummy.updateMatrix();
...
function _syncDrawCount() {
  let count = 0;
  for (let i = 0; i < CAP_PICKUPS; i++) {
    if (_phase[i] !== CP_FREE) count = i + 1;
  }
  _bodyMesh.count = count;
  _sparkleMesh.count = count;
}
