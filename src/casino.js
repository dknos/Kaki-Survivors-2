...
import { getMeta, saveMeta, setUnlockFlag } from './meta.js';
...
export function resolveSpin(bet, reels) {
  ...
  if (res.tier === 'jackpot') {
    ...
    // Unlock RockerKaki — this flag was defined + gated but never set anywhere
    // (dead flag, same class as the fixed unlockedVoid). Idempotent.
    try { setUnlockFlag('casinoJackpot'); } catch (_) {}
  }
  saveMeta();
