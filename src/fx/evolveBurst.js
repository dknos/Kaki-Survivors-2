...
/**
 * Build the 30s rim ring. Preferred: a flat PlaneGeometry carrying the
 * hand-painted `aoe_ascension` halo (tintable per-character via `color`) —
 * an ornate golden glow instead of the flat torus outline (the documented
 * plain-AoE anti-pattern). Falls back to the original TorusGeometry rim when
 * the manifest hasn't landed / the texture is missing, so the FX keeps
 * working with a working procedural rim. Attached to the scene and follows
 * the player (position set each tick).
 */
function _makeRimRing(color) {
  const ascTex = fxTex('aoe_ascension');
  if (ascTex) {
    // Flat halo plane on the XZ plane (rotated -90° around X via _flatX).
    // Plane spans ~1.5× the rim diameter so the baked halo ring lands near
    // RIM_INNER_R with room for its outer glow falloff (padding is a visual
    // judgment call — tune if the halo reads too tight/loose).
    const size = (RIM_INNER_R + RIM_LINE_W * 0.5) * 2 * 1.5;
    const geo = new THREE.PlaneGeometry(size, size);
    const mat = new THREE.MeshBasicMaterial({
      color,
      map: ascTex,
  ...
  // Procedural fallback: torus on the XZ plane. Radius = inner radius + half
  // line width so the visual reads as RIM_INNER_R.
