...
export function initDissolveBurst(scene) {
  ...
  // Hidden matrices still submit a fixed-count InstancedMesh. Avoid the
  // beauty+bloom draws until the first dissolve slot is actually leased.
  _inst.visible = false;
  _inst.frustumCulled = false;
  _inst.userData.visualRole = 'dissolve_burst_pool';
  _inst.userData.poolCapacity = POOL_CAP;
...
export function spawnDissolveBurst(pos, stageId) {
  ...
  // stopped scanning.
  if (written > 0) _inst.visible = true;
  return written;
}
...
export function tickDissolveBursts(dt) {
  ...
  if (_activeSlotCount === 0) _inst.visible = false;
}
