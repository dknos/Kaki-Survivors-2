...
export function tickVelocityVeils(dt) {
  if (!_inst || (_activeVeilCount === 0 && _activeSlotCount === 0)) return;

  // ── Pass 1: emit ribbon segments per live veil ──────────────────────────
  if (_activeVeilCount > 0) {
    for (let i = 0; i < MAX_VEILS; i++) {
      const d = _veils[i];
      if (!d.used) continue;
      const sp = d.statePtr;
...
export function disposeAllVelocityVeils(/* scene */) {
  for (let i = 0; i < POOL_CAP; i++) {
    ...
    if (!s.used) continue;
    s.used = false;
    if (_inst) _inst.setMatrixAt(i, _hideMat);
  }
  _activeSlotCount = 0;
  if (_inst) {
    _inst.count = 0;
