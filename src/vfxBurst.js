...
import { initAoeNova, updateAoeNova, resetAoeNova } from './fx/aoeNova.js';
import { initNovaBurst, updateNovaBurst, resetNovaBurst } from './fx/novaBurst.js';
...
function _mkInst(geo, mat, cap, flat = true) {
  ...
  for (let i = 0; i < cap; i++) {
    _m4.compose(_v3.set(0, -1000, 0), flat ? _flatX : _identityQ, _zeroScale);
...
export function initVFXBurst(scene) {
  ...
  const shockMat = new THREE.MeshBasicMaterial({
    // Generated white shockwave ring (tintable — callers pass cyan/red/gold via
    // per-instance color); falls back to the procedural canvas ring.
    map: fxTex('aoe_shockwave') || tex('shockwave'),
    transparent: true, opacity: 1.0,
    depthWrite: false, blending: THREE.AdditiveBlending,
  });
  ...
  // AoE nova pool piggybacks this lifecycle (init/update/reset) so every mode
  // that already ticks vfxBurst gets it with no main.js wiring.
  initAoeNova(scene);
  initNovaBurst(scene);
}
 ...
 */
export function spawnImpactBurst(x, y, z, color = 0xffb14a, intensity = 0.5, emberCap = 8) {
  const n = Math.min(emberCap, 4 + Math.floor(intensity * 4));   // 3..8 embers
...
export function updateVFXBurst(dt) {
  ...
  for (let i = 0; i < _flashes.length;) {
    const f = _flashes[i];
    f.t += dt;
    const k = f.t / f.life;
    if (k >= 1) {
      const last = _flashes.length - 1;
      if (i !== last) _flashes[i] = _flashes[last];
      _flashes.pop();
      continue;
    }
  ...
  _flashInst.count = _flashes.length;

  // Shockwaves — expand and fade flat on the ground.
  for (let i = 0; i < _shocks.length;) {
    const r = _shocks[i];
    r.t += dt;
    const k = r.t / r.life;
    if (k >= 1) {
      const last = _shocks.length - 1;
      if (i !== last) _shocks[i] = _shocks[last];
      _shocks.pop();
      continue;
    }
  ...
  _shockInst.count = _shocks.length;

  // Smoke — rise, expand, fade. Use flat orientation; reads as ground puff.
  const smokeXZDrag = Math.pow(0.4, dt);
  const smokeYDrag = Math.pow(0.6, dt);
  for (let i = 0; i < _smokes.length;) {
    const sm = _smokes[i];
    sm.t += dt;
    const k = sm.t / sm.life;
    if (k >= 1) {
      const last = _smokes.length - 1;
      if (i !== last) _smokes[i] = _smokes[last];
      _smokes.pop();
      continue;
    }
    ...
    sm.vx *= smokeXZDrag;
    sm.vz *= smokeXZDrag;
    sm.vy = Math.max(0.2, sm.vy * smokeYDrag);
  ...
  _smokeInst.count = _smokes.length;

  // Embers — ballistic with gravity + air drag, additive sparks.
  const emberDrag = Math.pow(_AIR, dt * 60);
  for (let i = 0; i < _embers.length;) {
    const e = _embers[i];
    e.t += dt;
    const k = e.t / e.life;
    if (k >= 1 || e.y < 0) {
      const last = _embers.length - 1;
      if (i !== last) _embers[i] = _embers[last];
      _embers.pop();
      continue;
    }
    e.vy += _GRAV * dt;
    e.vx *= emberDrag; e.vz *= emberDrag;
    e.x += e.vx * dt;
    e.y += e.vy * dt;
    e.z += e.vz * dt;
    const s = 0.6 * (1 - k * 0.3);
  ...
  _emberInst.count = _embers.length;
  ...
  for (let i = 0; i < _dashStreaks.length;) {
    const ds = _dashStreaks[i];
    ds.t += dt;
    const k = ds.t / ds.life;
    if (k >= 1) {
      const last = _dashStreaks.length - 1;
      if (i !== last) _dashStreaks[i] = _dashStreaks[last];
      _dashStreaks.pop();
      continue;
    }
  ...
  _dashInst.count = _dashStreaks.length;
