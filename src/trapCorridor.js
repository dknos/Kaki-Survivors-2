...
import { configureSharedEnvironmentMaterial } from './assets.js';
...
function _buildShardMesh(paletteSlots) {
  ...
  const mat = configureSharedEnvironmentMaterial(new THREE.MeshLambertMaterial({
...
function _applyTrapDamage(trap, hero, enemies) {
  ...
  let cands = enemies;
  // Check if spatial hash is passed or available via global state
  // Note: state is already imported at the top of the file via `import { state } from './state.js';`
  // So we can use `state.enemies.spatial` directly.
  // Also checking if the caller passed the full `state.enemies.active` list.
  // If the caller passed a filtered subset of enemies, we don't want to override it with
  // a spatial query that could return enemies outside that subset.
  const isFullActiveList = (state && state.enemies && enemies === state.enemies.active);
  const spatialHash = (state && state.enemies && state.enemies.spatial) || null;

  if (spatialHash && isFullActiveList && trap.radius) {
    cands = spatialHash.queryRadiusInto(trap.center, trap.radius, _trapNearby);
  }
  ...
  for (let i = 0; i < cands.length; i++) {
    ...
    try {
      // Trash dies to the trap (lethal) — strip its gem first (xp=0; killEnemy
      // skips the drop). A bot standing at spawn farmed 1012 trap kills for
      // 77% of its damage and ~19 free levels by t=59s: free kills must not
      // also be free XP. Elites keep gems (trap only chips 25% of their hp).
      if (!e.elite) e.xp = 0;
