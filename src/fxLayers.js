...
export const FLOOR_TIER = Object.freeze({
  ...
  terrain_hazard: -7, // persistent mud/lava/thorns; below transient signals
  player_field: -5, // persistent player-owned reach/weapon fields
 ...
 * @param {{bloom?: boolean}} [opts] Override bloom participation. Persistent
 *   player-owned floor fields use `false` so bloom blur cannot wash over the
 *   hero after the depth-tested base pass.
 */
export function applyFloorTier(mesh, tier, opts) {
  ...
  const bloom = opts && typeof opts.bloom === 'boolean'
    ? opts.bloom
    : tier !== 'shadow';
