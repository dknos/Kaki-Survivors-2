...
import { spawnEnemy, damageEnemy, releaseEnemyVisual } from './enemies.js';
...
import { initHelltide, tickHelltide, teardownHelltide, isHelltideActive, hasHelltideActivity, syncHelltidePromptVisibility } from './helltide.js';
...
export function initMiniEvents(scene) {
  _scene = scene;
  // Boot wires scene-owned pools without consuming the Town brazier's queued
  // next-run flag. The real run-entry reset below owns scheduling.
  resetMiniEvents({ scheduleHelltide: false });
}

export function resetMiniEvents({ scheduleHelltide = true } = {}) {
  ...
  try { initHelltide(_scene, { schedule: scheduleHelltide }); } catch (_) {}
}

/** True while scene-owned event art/timers make a Catacomb transition unsafe. */
export function isMiniEventBlockingCatacomb() {
  return _activeEvent !== null
    || hasHelltideActivity();
}

/** Keep Helltide proximity chrome out from under pause/reward/death modals. */
export function syncMiniEventUIVisibility(gameplayVisible) {
  syncHelltidePromptVisibility(gameplayVisible);
}
...
export function tickMiniEvents(dt) {
  ...
  if (!_scene) return;
  // Kaki Land owns its encounter cadence through its three visible portals.
  // Suppress Helltide and generic mini-events so nothing appears off-island
  // or obscures the portal route.
...
function _startGoblin() {
  ...
  _goblinState = { enemy: e, deadlineAt: overworldTime() + GOBLIN_KILL_WINDOW, origFlash };
  _activeEvent = 'goblin';
...
function _tickGoblin(dt, t) {
  ...
  if (t >= gs.deadlineAt || distSq > 80 * 80) {
    // Despawn + escape — release through the canonical visual seam so sprite
    // slots and GLB meshes return to their respective pools.
    e.alive = false;
...
function _spawnCache(x, z) {
  ...
  g.add(innerSpark);
  // Reuse the shared reward language for the goblin cache. A compact paw
  // marker says "collect me" without looking like a floor danger circle.
  const marker = createPickupMarker({
    size: 1.30,
    opacity: 0.86,
    gameplayPurpose: 'walk-over treasure goblin cache',
    userData: { miniEventPart: 'rewardMarker' },
  }).mesh;
  marker.position.y = 1.4;
  marker.userData.phase = Math.random() * Math.PI * 2;
  // Twinkle pip floating above the reward marker.
  ...
  g.add(body); g.add(marker); g.add(twinkle);
...
function _tickCachePickup() {
  ...
  for (let i = _caches.length - 1; i >= 0; i--) {
    ...
    if (marker) {
      ...
      const s = 1 + Math.sin(c.t * 2.6 + phase) * 0.055;
...
function _startMeteor() {
  ...
  for (let i = 0; i < METEOR_STRIKES; i++) {
    ...
    strikes.push({
      ...
      armAt: overworldTime() + METEOR_ARM_TIME + i * 0.08,
    });
  }
...
function _makeStrikeRing(radius) {
  ...
  const g = new THREE.Group();
  // Inner danger disc — baked red impact-crater art when available (aoe_crater
  // is a red-on-black additive webp, so tint white to show it true), else the
  // procedural flat red disc. Additive + depthWrite:false so the texture's
  // black bg drops out; tick still drives opacity as the strike arms.
  const craterTex = fxTex('aoe_crater');
  const inner = new THREE.Mesh(
    new THREE.CircleGeometry(radius * 0.9, 32),
    new THREE.MeshBasicMaterial({
      map: craterTex || null,
      color: craterTex ? 0xffffff : 0xff3a2a,
      transparent: true, opacity: 0.32, depthWrite: false,
      // Additive drops the baked art's black bg; the procedural fallback stays
      // NormalBlending so the solid red danger disc keeps its telegraph read.
      blending: craterTex ? THREE.AdditiveBlending : THREE.NormalBlending,
    })
  );
...
function _startElite() {
  ...
  _eliteState = {
    members,
    deadlineAt: overworldTime() + ELITE_KILL_WINDOW,
    tellRing,
    cx, cz,
...
function _approxDifficulty() {
  // Mirror of spawnDirector.computeDifficulty via the SPAWN constants — a
  // hardcoded copy capped at 7 went stale when the curve compressed to 660s
  // and ran elite mini-event tier picks soft against the real pipeline.
  const t = overworldTime();
