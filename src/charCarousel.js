 ...
 *
 * Self-contained renderer + scene that hosts real GLB models of every
...
import { createPreviewRendererAdapter } from './rendering/previewRendererAdapter.js';
...
export function createCharCarousel(host, opts) {
  ...
  // ── THREE init ──────────────────────────────────────────────────────
  // Inject this module's namespace so the preview always shares one Three.js
  // object universe with its scene. WebGPURenderer then asynchronously selects
  // either its WebGPU backend or the common WebGL 2 fallback.
  const previewRenderer = createPreviewRendererAdapter({
    THREE,
    mount: stage,
    ...
    canvasStyle: `
      ...
      z-index: 1;
    `,
    loadingText: 'Preparing hero preview…',
    errorText: 'Hero preview unavailable. Character selection remains usable.',
  });
  const canvas = previewRenderer.canvas;
  ...
  const resize = () => {
    ...
    previewRenderer.resize(w, h, false);
  ...
  function tick(now) {
    if (!_visible || _destroyed || previewRenderer.state === 'failed') {
      _raf = 0;
      return;
    }
    ...
    lastT = now;
    nextRenderAt = now + PREVIEW_FRAME_MS;
  ...
  return {
    ...
    refreshUnlocks,
    show: () => {
      if (_destroyed || _visible || previewRenderer.state === 'failed') return;
      _visible = true;
    ...
    whenRendererReady: () => previewRenderer.ready,
    getRendererDiagnostics: () => previewRenderer.getDiagnostics(),
    destroy: () => {
...
function _formatLockHint(unlock) {
  ...
  if (unlock.startsWith('flag:')) {
    ...
    if (f === 'unlockedClockwork') return 'Clear Boss Rush in Paper Woods to unlock.';
    return `Unlock: ${f}`;
  }
