...
import { createDpsWindow, resetDpsWindow } from './dpsWindow.js';

export const state = {
  // ── Top-level mode: 'menu' (start screen), 'town' (hub), 'run' (active game),
  //    'interior' (cabin room), 'catacomb' (dungeon sub-arena), 'racing'
  //    (self-contained Kaki Rally course). ──
  mode: 'menu',

  // Kaki Rally session. The racing module owns construction/disposal but keeps
  // the live session on shared state so resets, QA hooks, and the main loop all
  ...
  renderer: /** @type {any|null} */ (null),
  rendererService: /** @type {any|null} */ (null),
  ...
  run: {
    ...
    dmgTaken: 0,
    pickedGems: 0,
    portalShards: 0,     // explore-mode objective: shards collected (0..5)
    environmentSeed: null, // new seeded layout per run (replay seed when active)
    stageDiscovery: null,  // {id, found, total} for biome exploration loop
    caveEchoCredits: 0,    // Echo Crystals reduce the Cave vault kill gate
    stageSecretsBroken: 0, // dash-smash exploration milestone counter
    outcome: null,         // null | {kind:'victory'|'defeat', stageId, at}
    pipesGrapple: null,   // Pipes-only public HUD/touch status; owned by signature weapon
    catacombCleared: false, // one rewarded procedural descent per run
    _catacombShardGranted: false, // Cinder dungeon contributes at most one objective shard
    dungeonDepth: 0,       // current Catacomb floor (1-based while below)
    terrainLayoutReady: null, // renderer handshake for truthful terrain hazards
    _overworldPausedTime: 0, // Catacomb duration excluded from world schedules
    startedAt: 0,
    // Rolling DPS window: fixed 50x100ms bucket ring (no per-hit allocation).
    _dpsWin: createDpsWindow(),
...
export function resetState() {
  ...
  state.run.environmentSeed = null;
  state.run.stageDiscovery = null;
  state.run.caveEchoCredits = 0;
  state.run.stageSecretsBroken = 0;
  state.run.pipesGrapple = null;
  state.run.pipesGrapplePrimaryHits = 0;
  state.run.catacombCleared = false;
  state.run._catacombShardGranted = false;
  state.run.dungeonPhase = 'IDLE';
  state.run.dungeonDepth = 0;
  state.run.dungeonRoom = 0;
  state.run.dungeonRooms = 0;
  state.run.terrainLayoutReady = null;
  state.run._overworldPausedTime = 0;
  state.run._finaleCarry = null;   // explore→finale build carryover (portalShards)
  state.run._shardDrops = null;    // mini-boss earned-shard drop queue
  ...
  // Active-pity bookkeeping (weapons/index.js weaponChoices) — 3-pin budget
  // and the every-3rd-draft spacing counter, both per-run.
  state.run._activePityShown = 0;
  state.run._activeDrafts = 0;
  ...
  state.run.evolutionsThisRun = 0;   // count of evolutions this run (evolver achievement)
  resetDpsWindow(state.run._dpsWin);
  ...
  // Kaki Land is an isolated final-chapter route: three satellite portal
  // trials unlock the central Sovereign gate. It is per-run on purpose, so a
  // fresh visit always presents the complete route without leaking a prior
  // boss state through pooled scene objects.
  state.run.kakiLand = {
    trials: { ember: false, tide: false, bloom: false },
    mainPortalUnlocked: false,
    mainBossSpawned: false,
  };
  // Forest's normal-run objective is one explicit chain: use a Glade portal,
  // clear its sealed combat room, and repeat until the fixed Boss Gate opens.
  // forestSealedDoors fills the room table from FOREST_ROOMS on load; keeping
  // the shell here makes reset/restart ownership unambiguous and prevents a
  // prior run's active wave or gate state leaking into the next one.
  state.run.forestPortalTrials    = null;
  state.run.forestTrialActive     = false;
  state.run._forestPortalTransfer = null;
  ...
  state.run._cinematicSeen        = { miniboss: false, elite: false, finalboss: false, roomboss: false, reaper: false };
...
export function xpForLevel(lvl) {
  // Pure exponential starves the late arc: at growth 1.27 the L28 step costs
  // ~12.7k XP (~88s droughts in playtests). Past lateLevel the growth flattens
  // so the drumbeat survives to the final boss.
  const lateLvl = XP.lateLevel || Infinity;
  if (lvl <= lateLvl) return Math.ceil(XP.base * Math.pow(XP.growth, lvl - 1));
  return Math.ceil(
    XP.base * Math.pow(XP.growth, lateLvl - 1)
            * Math.pow(XP.lateGrowth || 1.1, lvl - lateLvl)
  );
}
