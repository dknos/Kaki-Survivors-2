...
import { sampleStageTerrain } from './stageTerrainLayout.js';
import {
...
const _hazardLoadGeneration = { forest: 0, twilight: 0, cinder: 0, void: 0 };

function _mkInst(geo, mat, cap, bloom = true) {
  const inst = new THREE.InstancedMesh(geo, mat, cap);
  // Only the authored roster prefix is submitted. Parking unused matrices
  // below the world still costs a draw and transforms every capacity slot.
  inst.count = 0;
  ...
  if (bloom) inst.layers.enable(BLOOM_LAYER);
  return inst;
}
...
export function initStageHazards(scene) {
  ...
  _pollenInst.userData.visualRole = 'stage_hazard_pool';
  _pollenInst.userData.hazardKind = 'pollen';
  ...
  const lavaMat = new THREE.MeshBasicMaterial({
    map: fxTex('aoe_lava') || tex('lavaPuddle'),
    color: 0xffffff, transparent: true, opacity: 0.95,
    depthWrite: false, blending: THREE.AdditiveBlending,
  });
  _lavaInst = _mkInst(lavaGeo, lavaMat, LAVA_CAP);
  _lavaInst.userData.visualRole = 'stage_hazard_pool';
  _lavaInst.userData.hazardKind = 'lava';
  scene.add(_lavaInst);
  // Twilight fog — a large dark plane that follows the hero and fades to clear
  // near the center via a world-space radial gradient.
  const fogGeo = new THREE.PlaneGeometry(120, 120);
  const fogMat = createTwilightFogMaterial();
  ...
  // Void chasms — actual missing floor, not a generic cyan AoE blob. A small
  // deterministic angular warp makes a jagged opaque-black center with a thin
  // dim cyan edge. Normal blending covers the ground; selective bloom stays
  // off so the hazard reads as a hole that absorbs light.
  const chasmGeo = new THREE.PlaneGeometry(2.0, 2.0);
  const chasmMat = createVoidChasmMaterial();
  _voidChasmInst = _mkInst(chasmGeo, chasmMat, VOID_CHASM_CAP, false);
  _voidChasmInst.name = 'voidChasmHazards';
  _voidChasmInst.userData.visualRole = 'stage_hazard_pool';
  _voidChasmInst.userData.hazardKind = 'void-chasm';
...
/** One authoritative query for the visible/damaging Cinder lava roster. */
export function isNearLiveLava(x, z, radius = 3) {
  const r2 = radius * radius;
  const now = state.time.game;
  for (let i = 0; i < _lavas.length; i++) {
    const lp = _lavas[i];
    if (!lp || lp.ttl <= 0 || now < lp.armingUntil) continue;
...
export function tickStageHazards(dt) {
  ...
  const terrainReady = !!(state.run && state.run.terrainLayoutReady === stageId);
  const terrain = terrainReady
    ? sampleStageTerrain(stageId, heroX, heroZ, _terrainSample)
    : Object.assign(_terrainSample, {
      inside: false, safe: false, active: false, kind: null,
      effect: null, slowMul: 1, damagePerSec: 0,
    });
  _terrainDamageCooldown = Math.max(0, _terrainDamageCooldown - dt);
  ...
  if (stageId === 'forest') {
    ...
    // Render + hero check. Submit only the roster prefix; when the existing
    // front-prune runs, the next frame naturally rewrites the shifted prefix.
    const pollenSubmitted = _pollens.length;
    ...
    while (_pollens.length > 0 && _pollens[0].ttl <= 0) _pollens.shift();
    _pollenInst.count = pollenSubmitted;
    if (pollenSubmitted > 0) {
  ...
  if (stageId === 'cinder') {
    ...
    const lavaSubmitted = _lavas.length;
    for (let i = 0; i < _lavas.length; i++) {
      const lp = _lavas[i];
      lp.ttl -= dt;
      if (lp.ttl <= 0) {
  ...
  if (stageId === 'void' && _voidChasms) {
    ...
    _voidChasmInst.count = 0;
  }

  // Authored ravines are truthful terrain, not decorative floor decals.
  // Their shared layout also owns the bridge OBBs, so the visible deck is an
  // exact safe crossing while water slows and lava/abyss damages elsewhere.
  let terrainSlow = 1.0;
  if (terrain.active) {
    if (terrain.effect === 'slow') terrainSlow = terrain.slowMul;
    else if (terrain.effect === 'damage' && terrain.damagePerSec > 0
      && _terrainDamageCooldown <= 0
      && state.time.game >= (state.hero.iFramesUntil || 0)) {
      try { heroTakeDamage(terrain.damagePerSec * 0.5); } catch (_) {}
      _terrainDamageCooldown = 0.5;
    }
  }

  // Apply pollen slow by multiplying hero speed for this frame.
  // Read by hero.js if we expose state.hero.hazardSlow.
  state.hero.hazardSlow = Math.min(pollenSlow, terrainSlow);
}

export function resetStageHazards() {
  for (const key of Object.keys(_hazardLoadGeneration)) _hazardLoadGeneration[key]++;
  _pollens.length = 0;
  _lavas.length = 0;
  _pollenCD = 0;
  _lavaCD = 0;
  _terrainDamageCooldown = 0;
  if (_pollenInst) _pollenInst.count = 0;
  if (_lavaInst) _lavaInst.count = 0;
  if (_voidChasmInst) _voidChasmInst.count = 0;
  _voidChasms = null;
...
export async function loadForestHazards(scene, hotspotsUrl = 'assets/forest_amber_hotspots.json') {
  ...
  const generation = ++_hazardLoadGeneration.forest;
  let hotspots = null;
  try {
  ...
  if (generation !== _hazardLoadGeneration.forest) return 0;
...
export async function loadTwilightHazards(scene, hotspotsUrl = 'assets/twilight_slowzone_hotspots.json') {
  ...
  const generation = ++_hazardLoadGeneration.twilight;
  let zones = null;
  try {
    const res = await fetch(hotspotsUrl);
  ...
  if (generation !== _hazardLoadGeneration.twilight) return 0;
...
export function clearTwilightHazards(_scene) {
  _hazardLoadGeneration.twilight++;
  _twilightSlowZones = null;
  if (state.run) state.run.twilightSlowZones = null;
}
...
export async function loadCinderHazards(scene, hotspotsUrl = 'assets/cinder_slowzone_hotspots.json') {
  ...
  const generation = ++_hazardLoadGeneration.cinder;
  let zones = null;
  try {
    const res = await fetch(hotspotsUrl);
    zones = await res.json();
  ...
  if (generation !== _hazardLoadGeneration.cinder) return 0;
  if (!Array.isArray(zones) || zones.length === 0) return 0;
  // Precompute r² so the enemy-loop hot path is mul-free per zone (JSON
  // stores r for human readability; runtime needs r²).
...
// Hotspots are mulberry32(0xC0DE99 ^ 0x40000). These are now the sole visible
// missing-floor roster: the former non-damaging arenaDecor decals were retired
// so every apparent hole has collision, damage, and a concrete purpose.
export async function loadVoidHazards(scene, hotspotsUrl = 'assets/void_chasm_hotspots_20260710.json') {
  // Idempotent: nuke any prior roster + hide instances before rebuilding.
  clearVoidHazards(scene);
  const generation = ++_hazardLoadGeneration.void;
  let hotspots = null;
  try {
    const res = await fetch(hotspotsUrl);
    hotspots = await res.json();
  ...
  if (generation !== _hazardLoadGeneration.void) return 0;
  if (!Array.isArray(hotspots) || hotspots.length === 0) return 0;
  if (!_voidChasmInst) {
  ...
  // Place instanced visuals flat on the tile plane. Geometry diameter is 2u,
  // so scale === collision radius and the black edge brackets damage exactly.
  ...
  _voidChasmInst.count = _voidChasms.length;
