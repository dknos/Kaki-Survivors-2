...
import { getMeta, AVATAR_UNLOCK_COSTS, isAvatarUnlocked } from './meta.js';
...
const BACKDROP_ID = 'kk-endrun-backdrop';

// Generated splash art (grok-imagine) — full-bleed backdrop behind the stats
// panel, keyed by outcome. Defeat art for death; victory art for win/outlast.
const SPLASH = {
  victory: { img: 'assets/screens/victory_crew_hangout_20260716.webp', title: 'VICTORY',  color: '#ffe08a' },
  outlast: { img: 'assets/screens/victory_crew_hangout_20260716.webp', title: 'SURVIVOR', color: '#ffe08a' },
  death:   { img: 'assets/screens/defeat.webp',  title: 'DEFEATED',  color: '#ff6a8a' },
};
...
export function tickEndRunSummary(state, _dt) {
  ...
  // Trigger A REMOVED (finale audit): every gameOver path already mounts the
  // full kk-death VICTORY/YOU DIED screen — popping this panel too stacked
  // two competing headers/CTAs over the 1.2-1.4s end anim and its capture-
  // phase Enter/Space handler stole the retry key. The summary now owns only
  // the reaper-outlast ending (below), which has no other end screen.
...
/**
 * Full-bleed splash backdrop behind the stats panel — the generated grok
 * victory/defeat art + a big outcome title, with a vignette + bottom scrim so
 * the panel stays readable. Keyed by reason; no-op for 'manual'.
 */
function _ensureBackdrop(reason) {
  if (typeof document === 'undefined') return;
  const prior = document.getElementById(BACKDROP_ID);
  if (prior && prior.parentNode) prior.parentNode.removeChild(prior);
  const spec = SPLASH[reason];
  if (!spec) return;

  const el = document.createElement('div');
  el.id = BACKDROP_ID;
  el.style.cssText = `
    position: fixed; inset: 0; z-index: 128; pointer-events: none;
    background: #05030a url('${spec.img}') center center / cover no-repeat;
  ...
  scrim.style.cssText = `
    ...
    background:
      radial-gradient(ellipse at center, rgba(0,0,0,0.05) 32%, rgba(0,0,0,0.55) 100%),
      linear-gradient(180deg, rgba(0,0,0,0.42) 0%, rgba(0,0,0,0) 24%,
                      rgba(0,0,0,0) 56%, rgba(0,0,0,0.74) 100%);
  `;
  ...
  title.style.cssText = `
    ...
    font-size: clamp(38px, 9vw, 92px); letter-spacing: 0.14em;
    color: ${spec.color};
    text-shadow: 0 4px 24px rgba(0,0,0,0.85), 0 0 44px ${spec.color}88;
  `;
...
function _ensureBuilt(reason) {
  ...
  // ── Next-unlock carrot (full width) — the "one more run?" hook ────────
  try { el.appendChild(_buildProgressSection()); } catch (_) {}
...
// Human hints for flag-gated cats (shown when the nearest locked cat isn't
// embers-gated). Mirrors the character-select unlock copy.
const _FLAG_HINTS = {
  firstMiniBossKill:  'Defeat a mini-boss',
  survive5MinRun:     'Survive 5 minutes in one run',
  catacombClear:      'Clear a catacomb dungeon',
  finalBossWin:       'Defeat a final boss',
  casinoJackpot:      'Hit the casino jackpot',
  allBossesHypermode: 'Sweep every boss on Hypermode',
};

// Full-width "Next Unlock" section on the results screen — surfaces the nearest
// locked character so the meta loop closes at the exact "one more run?" moment.
// Reads live meta only (no new run state). Prefers the closest embers-gated cat
// (has a satisfying progress bar); falls back to a flag hint, then "all done".
function _buildProgressSection() {
  let meta; try { meta = getMeta(); } catch (_) { meta = {}; }
  const embers = Math.max(0, (meta && meta.embers) || 0);

  const wrap = document.createElement('div');
  wrap.style.cssText = 'margin: 4px 0 16px; text-align: center;';
  const h = document.createElement('h3');
  h.textContent = 'Next Unlock';
  wrap.appendChild(h);

  const isUnlocked = (id) => { try { return isAvatarUnlocked(id); } catch (_) { return false; } };

  // Nearest LOCKED embers-gated cat (smallest cost = closest goal).
  let emberBest = null;
  for (const av of AVATARS) {
    const cost = AVATAR_UNLOCK_COSTS[av.id];
    if (!cost || cost.embers == null || cost.embers <= 0) continue;
    if (isUnlocked(av.id)) continue;
    if (!emberBest || cost.embers < emberBest.cost) emberBest = { av, cost: cost.embers };
  }

  const line = document.createElement('div');
  line.style.cssText = `font-size: 15px; font-weight: 700; letter-spacing: 0.04em; color: ${C_HEADER_WIN}; margin: 4px 0 8px; text-shadow: 0 1px 4px rgba(0,0,0,0.6);`;
  ...
  if (emberBest) {
    ...
    line.textContent = (need <= 0)
      ? `★ Ready! Unlock ${nm} in the character select`
      : `🔒 ${need} more Ember${need === 1 ? '' : 's'}  →  ${nm}`;
    wrap.appendChild(line);
    wrap.appendChild(_progressBar(pct, `${embers} / ${cost} Embers`));
  } else {
    // No embers cats left → nearest locked flag-gated cat, else all unlocked.
    let flagAv = null;
    for (const av of AVATARS) {
      const cost = AVATAR_UNLOCK_COSTS[av.id];
      if (!cost || !cost.flag) continue;
      if (isUnlocked(av.id)) continue;
      flagAv = av; break;
    }
    if (flagAv) {
      const cost = AVATAR_UNLOCK_COSTS[flagAv.id];
      const nm = `${flagAv.icon ? flagAv.icon + ' ' : ''}${(flagAv.name || flagAv.id).toUpperCase()}`;
      const hint = _FLAG_HINTS[cost.flag] || 'Complete a challenge';
      line.textContent = `🔒 ${hint}  →  ${nm}`;
      wrap.appendChild(line);
    } else {
      line.textContent = '★ Every cat unlocked — chase the Codex & achievements!';
      line.style.color = C_VALUE;
      wrap.appendChild(line);
    }
  }
  return wrap;
...
function _progressBar(pct, caption) {
  const box = document.createElement('div');
  box.style.cssText = 'max-width: 340px; margin: 0 auto;';
  const track = document.createElement('div');
  track.style.cssText = 'height: 10px; border-radius: 6px; background: rgba(8,14,12,0.6); box-shadow: 0 0 0 1px rgba(255,216,107,0.35) inset; overflow: hidden;';
  const fill = document.createElement('div');
  fill.style.cssText = `height: 100%; width: ${(pct * 100).toFixed(1)}%; border-radius: 6px; background: linear-gradient(90deg, ${C_VALUE}, ${C_HEADER_WIN}); box-shadow: 0 0 8px ${C_VALUE};`;
  track.appendChild(fill);
  box.appendChild(track);
  if (caption) {
    const cap = document.createElement('div');
    cap.style.cssText = `font-size: 11px; letter-spacing: 0.1em; color: ${C_LABEL}; margin-top: 5px;`;
