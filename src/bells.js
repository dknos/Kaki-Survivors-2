...
function _ringBell(b) {
  ...
  // Enemy combat timestamps use the raw combat clock even while the world
  // schedule is paused for a dungeon descent.
  ...
  // Audio: ominous toll on each ring — the audible half of the enrage telegraph.
  try { if (sfx && sfx.bellToll) sfx.bellToll(); } catch (_) {}
...
export function tickBells(dt) {
  ...
  if (overworldTime() < state.bells.armedAt) return;
