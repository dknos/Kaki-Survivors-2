...
function _makeInstanced(geo, color, emissiveColor, cap) {
  ...
  // Empty pickup pools should submit no instances. Previously all ten pickup
  // meshes drew their full capacity with zero-scale matrices even before the
  // first drop, which made quiet Forest frames needlessly expensive.
  inst.count = 0;
...
export function spawnHeart(x, z) {
  ...
  if (state.run && state.run.stage && state.run.stage.id === 'forest') {
    x = constrainForestX(x, 1);
    z = constrainForestZ(z, 1);
  }
...
export function tickPickups(dt) {
  ...
  // Logical lists are dense (spawn pushes / pickup splices), so draw count
  // itself hides trailing slots. This avoids 70 unnecessary matrix writes
  // every logic frame and removes all idle pickup instances from the GPU.
  _heartInst.count = _heartHaloInst.count = _hearts.length;
  _starInst.count = _starHaloInst.count = _stars.length;
  _bombInst.count = _bombHaloInst.count = _bombs.length;
  _freezeInst.count = _freezeHaloInst.count = _freezes.length;
  _chickenInst.count = _chickenHaloInst.count = _chickens.length;
