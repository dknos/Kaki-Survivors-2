...
import { advanceMaoMaoAfterRun, normalizeMaoMao } from './maomaoState.js';
import { normalizeBackendPreference } from './rendering/rendererSettings.js';
...
const DEFAULT = {
  ...
  // ── MaoMao's Daycare ──
  // First completed Hunt unlocks a short town rescue; adoption is performed
  // by the player. No offline decay. Each Hunt refreshes the care cycle.
  daycare: {
    version: 2, catId: 'maomao', name: 'MaoMao',
    encounterUnlocked: false, adopted: false, rescued: false, rescueStep: 0,
    adoptedAt: null, happiness: 0, energy: 3, bondXp: 0, careTotal: 0,
    careCycle: 0, careClaims: {}, unlockedOutfits: [], equippedOutfit: null,
    lastGiftDay: null, yarnBest: 0,
  },
  catacomb: { clears: 0, bestDepth: 0, bestFloor: 0, lastSeed: null, moonbellUnlocked: false },
  ...
  // Fine-pointer (desktop) defaults to 0.45 for readable hit feedback; coarse
  // (touch) keeps the quieter 0.25 baseline. Slider (0-1.5) still re-adjustable.
  // 0.45 not 0.6: with iFramesSec 0.40 hits re-arm shake up to 2.5 Hz —
  // 0.6 turned sustained swarm contact into continuous noise.
  optShake: (typeof matchMedia === 'function' && matchMedia('(pointer: coarse)').matches) ? 0.18 : 0.32,

  // Fresh saves only — existing saves carry an explicit stored value through
  // the {...DEFAULT,...parsed} merge in loadMeta, so stored=false users stay off.
  // Known edge (accepted): saves that PREDATE the optMusic key (incl. v1
  // migrations) have no stored value and are treated as fresh — music on.
  optMusic: true,
  ...
  // mutating the real campaign unlock flags, so switching it back off restores
  // the profile's genuine progression exactly as it was.
  optDevUnlockAllLevels: false,
  ...
  // Cinder's bounded five-wave portal finale opens Catacomb Void. The flag
  // also gates the Seedy Tent; a Void clear advances to Stonewright Caverns.
  unlockedVoid: false,
  unlockedCave: false,      // unlocked by victory on Catacomb Void
  unlockedKakiLand: false,  // unlocked by victory in Stonewright Caverns
  // ── Casino (iter 22B) ───────────────────────────────────────────────
...
/** Persistent high-value Catacomb reward + Town/Daycare bridge. */
export function grantCatacombReward(depth = 1, seed = null) {
  const meta = getMeta();
  if (!meta.catacomb || typeof meta.catacomb !== 'object') {
    meta.catacomb = { clears: 0, bestDepth: 0, bestFloor: 0, lastSeed: null, moonbellUnlocked: false };
  }
  const d = Math.max(1, Math.floor(Number(depth) || 1));
  meta.catacomb.clears = Math.max(0, Math.floor(meta.catacomb.clears || 0)) + 1;
  // `bestDepth` historically stored critical-room count. Preserve it for old
  // saves and track the new multi-floor progression in its own clean field.
  meta.catacomb.bestFloor = Math.max(Math.floor(meta.catacomb.bestFloor || 0), d);
  meta.catacomb.lastSeed = Number.isFinite(seed) ? (seed >>> 0) : null;
  const pet = normalizeMaoMao(meta);
  const moonbellNew = !pet.unlockedOutfits.includes('moonbell');
  if (moonbellNew) pet.unlockedOutfits.push('moonbell');
  meta.catacomb.moonbellUnlocked = true;
  saveMeta();
  return { clears: meta.catacomb.clears, bestDepth: meta.catacomb.bestDepth, bestFloor: meta.catacomb.bestFloor, moonbellNew };
}
 ...
 */
/**
 * The flag→avatar bridge. Flag-gated avatars (bomdia/camper/radcat/bezelbug/
 * rocker/borgirboss) had their unlock flags set by gameplay AND isAvatarUnlockable
 * returned true once satisfied — but NOTHING ever called unlockAvatar() for them,
 * so all 6 were permanently unreachable. This sweeps pure-flag avatars whose flag
 * is now satisfied into the avatarUnlocks map. Returns the ids unlocked this call.
 * Pure-flag only (cost.embers == null) so paid avatars stay behind their button.
 */
export function reconcileFlagAvatars() {
  const freshly = [];
  for (const id in AVATAR_UNLOCK_COSTS) {
    const cost = AVATAR_UNLOCK_COSTS[id];
    if (!cost || !cost.flag || cost.embers != null) continue;
    if (isAvatarUnlocked(id) || !isAvatarUnlockable(id)) continue;
    unlockAvatar(id, 'flag:' + cost.flag);
    freshly.push(id);
  }
  return freshly;
}
...
export function setUnlockFlag(name) {
  ...
  saveMeta();
  // Live path: a newly-set flag may satisfy a flag-gated avatar — unlock it now
  // and emit an event so the UI can celebrate (listener is a follow-up).
  try {
    const freshly = reconcileFlagAvatars();
    if (freshly.length && typeof window !== 'undefined') {
      for (const id of freshly) window.dispatchEvent(new CustomEvent('kk-avatar-unlocked', { detail: { id, via: name } }));
    }
  } catch (_) {}
  return true;
}
...
export const ACHIEVEMENTS = [
  ...
  {
    id: 'champion_twilight', name: 'Folded Weather',
    desc: 'Victory in Paper Woods',
    icon: '🌒',
...
function migrateV1ToV2(v1) {
  ...
  out.optRenderer   = normalizeBackendPreference(out.optRenderer);
...
export function loadMeta() {
  ...
  try {
    ...
    if (rawV2) {
      ...
      _data.optRenderer = normalizeBackendPreference(_data.optRenderer);
...
export function saveMeta() {
  ...
  try {
    // Direct meta mutation is retained for legacy callers, so normalize here
    // as well as in setOption/import/load before anything reaches disk.
    _data.optRenderer = normalizeBackendPreference(_data.optRenderer);
...
let _flagAvatarsReconciled = false;
export function getMeta() {
  const m = _data || loadMeta();
  // Retroactive boot sweep (once): existing profiles whose flags are already set
  // won't re-fire setUnlockFlag, so unlock their earned flag-avatars now. Silent
  // (no events) to avoid a toast-storm on load. Guard flips BEFORE the sweep so
  // the unlockAvatar→getMeta re-entry can't recurse.
  if (!_flagAvatarsReconciled) {
    _flagAvatarsReconciled = true;
    try { reconcileFlagAvatars(); } catch (_) {}
  }
  return m;
}
...
export function commitRunResults({ timeSurvived, kills, dmgDealt, level, victory, stageId, bossRush, weekly, daily, character, fullSweep }) {
  ...
  // ×1.5 victory premium: beating the final boss used to pay the same coins
  // as dying at the same second — the win needs a meta incentive over
  // farming endless.
  const coinMul = (meta.optHyper ? 1.5 : 1) * (1 + 0.25 * vaultLv) * (1 + greedMul + passiveCoinMul) * dailyMul * (victory ? 1.5 : 1);
  ...
  let unlockedHyper = false, unlockedEndless = false, unlockedCinder = false, unlockedClockwork = false, unlockedVoid = false, unlockedCave = false, unlockedKakiLand = false;
  ...
  // Catacomb Void is opened by Cinder's bounded finale. Clearing Void advances
  // the campaign to Stonewright Caverns and opens the Seedy Tent.
  if (victory && stageId === 'void') {
    if (!meta.unlockedVoid) { meta.unlockedVoid = true; unlockedVoid = true; }
    if (!meta.unlockedCave) { meta.unlockedCave = true; unlockedCave = true; }
    meta.casinoUnlocked = true;
  }
  // Stonewright Caverns completes the overworld campaign route and opens the
  // optional final chapter. This is deliberately separate from the older
  // Cinder → Void handoff, which also unlocks the Seedy Tent.
  if (victory && stageId === 'cave' && !meta.unlockedKakiLand) {
    meta.unlockedKakiLand = true;
    unlockedKakiLand = true;
  }
  ...
  // ── Achievement DAG sweep — fixes the 9-of-17 dead tier-2/3 chain ─────────
  // These entries carry progress()/binary conditions but NOTHING ever unlocked
  // them (the live path is a separate forestAchievements system), so the codex
  // capped at 8/17 and all 3 capstones were unreachable — a bar that fills to
  // 100% and never completes. Sweep now that every counter for this run is
  // persisted. Binary victory conditions feed via `_cond`; progress() entries
  // evaluate against meta. Requires-gated + iterated to a fixpoint so a parent
  // unlocks before its child is tested (apex_predator ⇐ centurion_x10,
  // void_walker ⇐ champion_twilight, master_collector ⇐ evolver ⇐ first_evolution).
  try {
    const _cond = {
      champion_twilight: !!(victory && stageId === 'twilight'),
      void_walker:       !!(victory && stageId === 'void'),
      evolver:           !!(state && state.run && (state.run.evolutionsThisRun || 0) >= 3),
      master_collector:  !!(meta.codex && meta.codex._allEvosDiscovered),
    };
    const _ach = meta.achievements || (meta.achievements = {});
    const _met = (a) => (typeof a.progress === 'function') ? a.progress(meta) >= 1 : !!_cond[a.id];
    for (let pass = 0; pass < 3; pass++) {
      let changed = false;
      for (const a of ACHIEVEMENTS) {
        if (_ach[a.id]) continue;
        if ((a.requires || []).some((req) => !_ach[req])) continue;
        if (!_met(a)) continue;
        if (unlockAchievement(a.id)) changed = true;
      }
      if (!changed) break;
    }
  } catch (_) {}

  // MaoMao — the first completed Hunt reveals paw prints in town. The player
  // performs the actual rescue using the town's dash/jump trail. Every later
  // Hunt refreshes one full round of care and restores one play energy.
  const maoProgress = advanceMaoMaoAfterRun(meta);
  ...
  return {
    ...
    unlockedVoid, unlockedCave, unlockedKakiLand,
    masteryGained, avatarId,
    weeklyResult,
    dailyWin, dailyBadgeUnlocked,
    maomaoEncounterUnlocked: maoProgress.encounterNew,
    rescuedKitten: false, // legacy summary key: adoption is no longer automatic
  };
}
...
export function setOption(key, val) {
  const meta = getMeta();
  meta[key] = key === 'optRenderer' ? normalizeBackendPreference(val) : val;
  saveMeta();
}

/**
 * Daily come-back gift — MaoMao leaves a few embers once per real-world
 * day (cozy retention hook; purely positive — no decay, no penalty for skipping
 * days). Called from town.js#enterTown on the first town visit each day. Gated
 * on adoption (no cat → no gift) and gently scaled by happiness. The old
 * 15–40 payout overwhelmed the intended 3–6-ember run economy; this 3–7 gift
 * stays delightful without making login rewards more valuable than play. Returns
 * the amount granted, or 0 if there's no cat / already claimed today.
 */
export function claimDailyGift() {
  const meta = getMeta();
  const pet = normalizeMaoMao(meta);
  if (!pet.adopted) return 0;
  const today = new Date().toISOString().slice(0, 10);   // UTC day-string
  if (pet.lastGiftDay === today) return 0;                // already claimed today
  const happiness = pet.happiness | 0;
  const embers = 3 + Math.floor(happiness / 25);          // 3 → 7
  meta.embers = (meta.embers | 0) + embers;
  pet.lastGiftDay = today;
  saveMeta();
  return embers;
}
...
/**
 * True when a stage is selectable for this profile. The developer override is
 * deliberately read as a presentation/runtime bypass; it never writes the
 * underlying campaign flags and is therefore fully reversible.
 */
export function isStageUnlocked(stage, meta = getMeta()) {
  if (!stage || !stage.unlock) return true;
  return !!(meta && (meta.optDevUnlockAllLevels || meta[stage.unlock]));
}

/** Resolve the currently-selected unlocked stage. Falls back to the first. */
export function selectedStage(STAGES) {
  const meta = getMeta();
  const selected = STAGES.find(s => s.id === meta.selectedStage);
  return (selected && isStageUnlocked(selected, meta)) ? selected : STAGES[0];
}
