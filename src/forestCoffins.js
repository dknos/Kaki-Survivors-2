 ...
 *   1 UNLOCKABLE  — padlock hidden, paw reward marker breathes overhead, lid
 ...
 *   shared paw marker + opening burst signal the reward state
 *
 * Constraints honored:
...
function _buildCoffinMeshes() {
  ...
  // The evolution reward is signposted by the same paw badge as other
  // walk-to rewards. It replaces the legacy spinning gold donut.
  const marker = createPickupMarker({
    size: 1.18,
    opacity: 0.88,
    instanced: true,
    cap: CAP_COFFINS,
    gameplayPurpose: 'walk-to evolution coffin',
    userData: { coffinKind: 'rewardMarker' },
  });
  _coffinSparkMesh = marker.mesh;
...
function _stampLockedMatrices(i) {
  ...
  // Reward marker — hidden on LOCKED (zero matrix).
  _coffinSparkMesh.setMatrixAt(i, _ZERO_MATRIX);
}

// Re-stamp matrices for a single coffin in UNLOCKABLE state (lid bobs,
// padlock hidden, paw marker visible + breathing). Called per-frame for
// unlockable coffins.
function _stampUnlockableMatrices(i) {
  ...
  // Padlock — hidden.
  _coffinLockMesh.setMatrixAt(i, _ZERO_MATRIX);
  // Compact reward badge at y=1.6. A slight yaw drift/pulse preserves a
  // living magical read without becoming a target-shaped circle.
  const yawDrift = yaw + Math.sin(_animClock * 1.35 + i) * 0.08;
  const pulse = 1 + Math.sin(_animClock * 2.6 + i) * 0.055;
  _dummy.position.set(x, 1.6, z);
  _dummy.rotation.set(0, yawDrift, 0);
  ...
  for (let i = 0; i < _coffinCount; i++) {
    ...
    if (st === ST_LOCKED) {
      if (eligibleIdx >= 0) {
        ...
        // Stamp the new pose (lid bob, padlock hidden, reward marker shown).
