 ...
 *
 * 1.5s camera dolly + name banner when the FIRST miniboss / elite / final boss /
 * room-boss / Reaper of a run spawns. One-shot PER TIER per run.
 *
 * ── Trigger sources ────────────────────────────────────────────────────────
 ...
 * @param {'miniboss'|'elite'|'finalboss'|'roomboss'|'reaper'} tier
 */
export function triggerBossIntro(enemyOrMesh, tier) {
  const state = _stateRef;
  if (!state || !state.run) return;
  if (!enemyOrMesh) return;
  if (tier !== 'miniboss' && tier !== 'elite' && tier !== 'finalboss' && tier !== 'roomboss' && tier !== 'reaper') return;
  ...
  let name = 'BOSS';
  if (enemyOrMesh && typeof enemyOrMesh.displayName === 'string' && enemyOrMesh.displayName.length > 0) {
    name = enemyOrMesh.displayName.toUpperCase();
  } else if (enemyOrMesh && typeof enemyOrMesh.glbKey === 'string' && enemyOrMesh.glbKey.length > 0) {
    name = enemyOrMesh.glbKey.toUpperCase();
  }
  const label = tier === 'reaper'
    ? 'INVINCIBLE: REAPER'
    : (tier === 'finalboss' ? `FINAL ENCOUNTER: ${name}` : `ENCOUNTER: ${name}`);
...
function _activeFallback(enemyOrMesh, tier) {
  ...
  if (_fallbackHideTO) clearTimeout(_fallbackHideTO);
  _fallbackHideTO = setTimeout(() => {
    _fallbackHideTO = null;
...
export function disposeBossIntroCinematic() {
  // Release HUD ownership before dropping the state reference. Otherwise a
  // stage swap during the 1.5s intro can leave every lower-priority notice
  // suppressed for the rest of the run.
  if (_stateRef && _stateRef.run) _stateRef.run._bossIntroActive = false;
  if (_fallbackHideTO) {
    clearTimeout(_fallbackHideTO);
    _fallbackHideTO = null;
  }
...
function _ensureStyle() {
  ...
  s.textContent = `
    #${BANNER_ID} {
      ...
      font-weight: 900;
      line-height: 1.15;
      letter-spacing: clamp(0.09em, 0.42vw, 0.18em);
      padding: clamp(8px, 1.4vh, 10px) clamp(14px, 2.5vw, 24px);
      background: linear-gradient(180deg, rgba(10,14,18,0.78), rgba(4,8,12,0.86));
      border-top: 1px solid rgba(0,0,0,0.85);
      border-bottom: 1px solid rgba(0,0,0,0.85);
      box-shadow: 0 10px 30px rgba(0,0,0,0.55);
      max-width: min(88vw, 860px); box-sizing: border-box; text-align: center; white-space: normal;
