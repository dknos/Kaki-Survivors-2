...
function _ensureStyle() {
  ...
  s.textContent = [
    ...
    '  gap: 10px;',
    '  padding: 7px 9px;',
    '  background: linear-gradient(145deg, rgba(44,29,39,.93), rgba(20,15,22,.94));',
    '  border: 1px solid var(--kk-hud-line, rgba(255,226,188,.30));',
    '  border-top-color: rgba(255,245,226,.40); border-radius: 12px;',
    '  box-shadow: 0 12px 28px rgba(13,7,11,.45), inset 0 1px 0 rgba(255,255,255,.07);',
    '  backdrop-filter: blur(12px) saturate(125%); -webkit-backdrop-filter: blur(12px) saturate(125%);',
    '  box-sizing: border-box;',
    '  opacity: 1;',
    '}',
    '#' + ROOT_ID + '.kk-bb-kaki .' + ROW_CLS + ' {',
    '  width: min(570px, 92vw);',
    '  padding: 8px 11px;',
    '  background: linear-gradient(145deg, rgba(38,24,42,.95), rgba(17,13,26,.96));',
    '  border-color: rgba(255,216,107,.54);',
    '  box-shadow: 0 14px 34px rgba(11,6,16,.52), 0 0 24px rgba(200,123,255,.12), inset 0 1px 0 rgba(255,255,255,.08);',
    '}',
    '#' + ROOT_ID + '.kk-bb-kaki .kk-bb-name { flex-basis: 180px; font-family: "Cinzel Decorative", serif; }',
    '#' + ROOT_ID + '.kk-bb-kaki .kk-bb-hp { flex-basis: 150px; }',
    '#' + ROOT_ID + ' .' + ROW_CLS + '.kk-bb-show {',
    '  display: flex;',
    '}',
    '#' + ROOT_ID + ' .kk-bb-name {',
    '  flex: 0 0 112px;',
    '  font-family: "Geist", sans-serif; font-size: 10px; font-weight: 800;',
    '  letter-spacing: 0.13em; text-transform: uppercase;',
    '  color: var(--kk-hud-cream, #fff0dc);',
    ...
    '  box-shadow: inset 0 1px 0 rgba(255,255,255,.45), 0 0 10px rgba(255,190,90,.24);',
    '  transition: none;',
    '}',
    '#' + ROOT_ID + ' .kk-bb-hp {',
    '  flex: 0 0 104px;',
    ...
    '  letter-spacing: .04em;',
    '  color: var(--kk-hud-muted, rgba(255,240,220,.64));',
    ...
    '  #' + ROOT_ID + ' { top: 74px; }',
    '  #' + ROOT_ID + ' .' + ROW_CLS + ' { gap: 6px; padding: 6px 7px; }',
    '  #' + ROOT_ID + ' .kk-bb-name { flex-basis: 76px; font-size: 8px; }',
    '  #' + ROOT_ID + ' .kk-bb-hp { flex-basis: 78px; font-size: 8px; }',
    '  #' + ROOT_ID + '.kk-bb-kaki .kk-bb-name { flex-basis: 105px; }',
    '  #' + ROOT_ID + '.kk-bb-kaki .kk-bb-hp { flex-basis: 100px; }',
    '}',
  ].join('\n');
...
function _fillColorForTier(tier, enemy = null) {
  if (enemy && enemy.kakiLandPortalId === 'kaki-ember') return '#ff704f';
  if (enemy && enemy.kakiLandPortalId === 'kaki-tide') return '#62dcff';
  if (enemy && enemy.kakiLandPortalId === 'kaki-bloom') return '#c789ff';
  if (enemy && enemy.kakiLandPortalId === 'kaki-main') {
    return 'linear-gradient(90deg, #ff704f 0 32%, #62dcff 34% 65%, #c789ff 67% 100%)';
  }
...
function _nameForEnemy(enemy) {
  ...
  if (!enemy) return '';
  if (typeof enemy.displayName === 'string' && enemy.displayName.length > 0) return enemy.displayName.toUpperCase();
...
function _applyTierStyling(row, tier, enemy = null) {
  const key = _tierKey(tier);
  const color = _fillColorForTier(tier, enemy);
  if (row.lastTier === key && row.lastFill === color) return;
  row.lastTier = key;
  row.lastFill = color;
  // Fill color
...
function _shouldShow(state) {
  ...
  if (state.gameOver === true) return false;
  if (state.run && state.run._bossIntroActive) return false;
  if (!state.run || !state.run.stage) return false;
  if (state.run.stage.id !== 'forest' && state.run.stage.id !== 'kakiland') return false;
  return true;
}
...
export function tickForestBossBars(state, dt) {
  ...
  const isKaki = !!(state && state.run && state.run.stage && state.run.stage.id === 'kakiland');
  _root.classList.toggle('kk-bb-kaki', isKaki);
  ...
  for (let ti = 0; ti < desiredN; ti++) {
    ...
    row.el.classList.remove('kk-bb-dying');
    _applyTierStyling(row, tgt.tier, tgt.enemy);
  }
  ...
  for (let i = 0; i < _rows.length; i++) {
    ...
    _applyTierStyling(row, tier, isReaper ? null : row.target);
    ...
    if (isReaper) {
      ...
      const phase = Number.isFinite(e._kakiEncounterPhase) ? e._kakiEncounterPhase + 1 : 0;
      const adds = Math.max(0, e._kakiEncounterAddsAlive || 0);
      const hpText = e._encounterInvulnerable
        ? `WARD · ${adds} ADD${adds === 1 ? '' : 'S'}`
        : (phase > 0 ? `PHASE ${phase} · ${hpNow} / ${hpMax}` : hpNow + ' / ' + hpMax);
      if (hpText !== row.lastHp) {
        row.hpEl.textContent = hpText;
        row.hpEl.style.color = e._encounterInvulnerable ? '#ffe6a8' : HP_BONE_CSS;
        row.lastHp = hpText;
      }
    }
