...
export function onPylonKilled(pylon) {
  ...
  state.pylons.respawnQueue.push({ at: overworldTime() + RESPAWN_DELAY });
}
...
export function tickPylons(dt) {
  ...
  if (overworldTime() < state.pylons.armedAt) return;
  ...
  for (let i = state.pylons.respawnQueue.length - 1; i >= 0; i--) {
    if (overworldTime() >= state.pylons.respawnQueue[i].at) {
      _spawnOne();
