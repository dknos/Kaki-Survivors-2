...
export async function loadTwilightFountains(scene, hotspotsUrl = 'assets/twilight_fountain_hotspots.json') {
  ...
  // Static stone parts are batched; only each liquid disc needs an independent
  // material/state machine. Six fountains now cost two static submissions
  // instead of twelve separate body/rim meshes.
  const bodyInst = new THREE.InstancedMesh(bodyGeo, bodyMat, hotspots.length);
  const rimInst = new THREE.InstancedMesh(rimGeo, rimMat, hotspots.length);
  bodyInst.name = 'twilightFountainBodies';
  rimInst.name = 'twilightFountainRims';
  bodyInst.castShadow = rimInst.castShadow = false;
  bodyInst.receiveShadow = rimInst.receiveShadow = true;
  const staticDummy = new THREE.Object3D();
  _group.add(bodyInst, rimInst);
  _disposables.push(bodyInst, rimInst);
  ...
  for (let hi = 0; hi < hotspots.length; hi++) {
    ...
    staticDummy.position.set(h.x, 0.275 * s, h.z);
    staticDummy.scale.setScalar(s);
    staticDummy.rotation.set(0, 0, 0);
    staticDummy.updateMatrix();
    bodyInst.setMatrixAt(hi, staticDummy.matrix);
    staticDummy.position.y = 0.55 * s;
    staticDummy.updateMatrix();
    rimInst.setMatrixAt(hi, staticDummy.matrix);
...
export function clearTwilightFountains(scene) {
  ...
  // Option A buff cleanup: use the canonical state singleton. The game does
  // not publish state on globalThis, so the former global probe left a buff
  // alive when game time reset between runs.
  if (gameState.run) gameState.run.fountainSpeedBuff = null;
}
