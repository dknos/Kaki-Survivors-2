...
let _starCharges = 0;       // banked by Star Kittens; next pad gets a 1s cooldown
...
// Readable in the base pass without turning every idle pad into a screen-sized
// bloom cloud. Teleport activation still owns the bright flash ring.
export const IDLE_EMISSIVE_MIN = 0.12;
export const IDLE_EMISSIVE_MAX = 0.28;
...
function _spawnCooldownRing(parentGroup) {
  ...
  mesh.visible = false;                 // zero-opacity idle rings cost no draw
...
export async function loadVoidTeleportPads(scene, hotspotsUrl = 'assets/void_teleport_hotspots.json') {
  ...
  for (const h of hotspots) {
    ...
    // Idle pads stay out of selective bloom; emissive color + chrome rim are
    // enough to identify the interaction. Activation flash rings still bloom.
...
export function tickVoidTeleportPads(dt, state) {
  ...
  for (const pad of _pads) {
    ...
    if (pad.cooldownActive) {
      ...
      if (remaining > 0) {
        ...
        if (pad.cooldownRing) {
          pad.cooldownRing.mesh.visible = true;
          pad.cooldownRing.mat.opacity = COOLDOWN_RING_OPACITY_MAX * frac;
        }
      } else {
        // Cooldown expired this tick — snap overlay off, fire ready chime ONCE.
        if (pad.cooldownRing) {
          pad.cooldownRing.mat.opacity = 0;
          pad.cooldownRing.mesh.visible = false;
        }
  ...
  if (heroAlive && heroPos) {
    for (const pad of _pads) {
      ...
      // (5) Cooldown on ORIGIN only (destination stays ready). A rescued Star
      // Kitten banks one useful charge even when the player found it long after
      // the previous 6s cooldown expired: the NEXT origin cools in only 1s.
      const starCharged = _starCharges > 0;
      if (starCharged) _starCharges--;
      pad.cooldownUntil = tNow + (starCharged ? 1.0 : COOLDOWN_DURATION);
      pad.cooldownActive = true;
      if (pad.cooldownRing) {
        pad.cooldownRing.mesh.visible = true;
        pad.cooldownRing.mat.opacity = COOLDOWN_RING_OPACITY_MAX;
      }
...
/**
 * Star-kitten discoveries feed their light back into the travel network.
 * Cooldowns are shortened in place; ready pads remain ready and no teleport
 * is forced, so the reward never steals player control.
 */
export function rechargeVoidPads(seconds = 2.5) {
  const amount = Math.max(0, Number(seconds) || 0);
  let changed = 0;
  for (const pad of _pads) {
    if (!pad || pad.cooldownUntil <= 0) continue;
    pad.cooldownUntil = Math.max(0, pad.cooldownUntil - amount);
    changed++;
  }
  _starCharges = Math.min(6, _starCharges + 1);
  return changed;
}
...
export function _debugStarCharges() { return _starCharges; }
