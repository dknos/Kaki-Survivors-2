...
// Keep the manifest on the same release epoch as index.html. Without a query
// tag, a returning Pages client can retain an older key table while loading
// new modules, causing authored textures to fall through to placeholders.
const MANIFEST_URL = 'assets/fx/MANIFEST.json?v=20260714d';
...
export function fxTier(tierName) {
  if (!_manifest || !_manifest._floor_tiers) {
    const fallback = { shadow: -10, terrain_hazard: -7, player_field: -5, kill_pickup: -5, portal: -4, telegraph: -3, boss_tell: -2 };
