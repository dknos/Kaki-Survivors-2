...
export function disposeEvolveCinematic() {
  if (_stateRef && _stateRef.run) _stateRef.run._evolveCinematicActive = false;
  if (_fallbackHideTO) {
...
function _ensureStyle() {
  ...
  s.textContent = `
    #${BANNER_ID} {
      ...
      font-weight: 900;
      line-height: 1.15;
      letter-spacing: clamp(0.09em, 0.4vw, 0.20em);
      padding: clamp(8px, 1.4vh, 12px) clamp(14px, 2.6vw, 26px);
      background: linear-gradient(180deg, rgba(10,14,18,0.78), rgba(4,8,12,0.86));
      border-top: 1px solid rgba(0,0,0,0.85);
      border-bottom: 1px solid rgba(0,0,0,0.85);
      box-shadow: 0 10px 30px rgba(0,0,0,0.55);
      max-width: min(88vw, 820px); box-sizing: border-box; text-align: center; white-space: normal;
