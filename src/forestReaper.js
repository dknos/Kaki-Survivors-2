...
/** Scene-owned Reaper/tint/pillar state that must finish before a descent. */
export function isForestReaperBlockingCatacomb() {
  const tintOpacity = _tintEl ? Number.parseFloat(_tintEl.style.opacity || '0') : 0;
  return !!(_alive || _pillarMesh || tintOpacity > 0.01);
}
  ...
  if (_alive && !state.gameOver && state.hero && state.hero.pos) {
    ...
    if ((dx * dx + dz * dz) <= CONTACT_R2) {
      ...
      if (state.run.stats.reaperKillTime == null) {
        state.run.stats.reaperKillTime = t;
      }
