...
const WEAPON_BLURBS = {
  ...
  sig_pipes_arcwrench: {
    flavor: 'Arc Grappler — Bolt the crowd, hook one troublemaker, then turn it into the projectile.',
    body: (lv) => {
      const p = Math.max(0, Math.min(1, (lv.dmg - 12) / 48));
      const range = 10 + 5 * p;
      const impact = Math.round(30 + 80 * p);
      return `Auto-fires a ${lv.pierce}-pierce arc bolt. Hold RMB to grab within ${range.toFixed(1)}m; shoot the captive to charge, then release for up to ${impact} collision damage.`;
    },
    tags: ['Grapple', 'Pierce', 'Skill'],
  },
  sig_bomdia_sunburst: {
...
const FILLER_BLURBS = {
  ...
  dash:     { flavor: 'Charge Dash — SHIFT teleport that batters foes aside. Ranks up automatically as you level.', tags: ['Mobility', 'Damage'] },
};
