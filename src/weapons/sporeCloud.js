...
function _ensureMeshes() {
  ...
  _cloudMat = new THREE.MeshBasicMaterial({
    // Generated toxic spore cloud (green-tinted via COLOR_CLOUD); procedural fallback.
    map: fxTex('aoe_poison') || tex('pollen') || tex('glowWhite'),
    color: COLOR_CLOUD,
    transparent: true,
    opacity: 0.65,
    depthWrite: false,
...
export default {
  ...
  dispose() {
    for (let i = 0; i < CLOUD_CAP; i++) {
      ...
      c.age = 0;
      c.recentTicks.clear();
      if (_cloudMesh) _hideCloud(i);
    }
    if (_cloudMesh) _cloudMesh.instanceMatrix.needsUpdate = true;
    if (_cloudMat) _cloudMat.opacity = 0.65;
  },
};
