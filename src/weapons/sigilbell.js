...
function _getSigilRuneTex() {
  return _sigilRuneTex || (_sigilRuneTex = fxTex('ring_arcane') || makeRuneRingTexture());
}
...
function _makeSigilMesh() {
  ...
  applyFloorTier(rune, 'telegraph', { bloom: false });
...
export default {
  ...
  dispose(state, inst) {
    if (!inst.sigils) return;
    for (const sigil of inst.sigils) _cleanupSigil(sigil, state.scene);
    inst.sigils.length = 0;
  },
};
