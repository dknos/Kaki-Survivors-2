...
function _ensureMeshes() {
  ...
  const patchMat = new THREE.MeshBasicMaterial({
    map: fxTex('aoe_field') || tex('glowWhite'),
    color: COLOR_PATCH,
    transparent: true,
    opacity: 0.78,
    depthWrite: false,
...
// ─── spawn-flash ring (slot 7, bloom) ────────────────────────────────────────
// Generated thorn-tangle burst (aoe_roots) on a flat plane; procedural glow
// fallback keeps it working before the manifest lands. Replaces the old flat
// RingGeometry+MeshBasicMaterial (the documented plain-AoE anti-pattern).
function _spawnFlashRing(x, z) {
  const outer = FLASH_INNER_R + FLASH_LINE_W;
  const rootTex = fxTex('aoe_roots');
  const geo = new THREE.PlaneGeometry(outer * 3.6, outer * 3.6);
  geo.rotateX(-Math.PI / 2);
  const mat = new THREE.MeshBasicMaterial({
    map: rootTex || tex('glowWhite'),
    color: rootTex ? 0xffffff : COLOR_FLASH,
    transparent: true,
    opacity: 1.0,
    side: THREE.DoubleSide,
    depthWrite: false,
...
export default {
  ...
  dispose() {
    ...
    if (_patchMesh) {
      _patchMesh.instanceMatrix.needsUpdate = true;
      if (_patchColorIdle) _patchMesh.material.color.copy(_patchColorIdle);
    }
    for (let i = 0; i < _flashRings.length; i++) {
      const fr = _flashRings[i];
      if (fr.mesh.parent) fr.mesh.parent.remove(fr.mesh);
      try { fr.geo.dispose(); } catch (_) {}
      try { fr.mat.dispose(); } catch (_) {}
