...
function _ensureMesh() {
  ...
  // aoe_lava is baked-orange (tintable:false) — show it true with a white tint;
  // the legacy lavaPuddle/emberWarm fallback still wants the warm 0xff7a3a wash.
  const lavaTex = fxTex('aoe_lava');
  const mat = floorDecalMaterial({ map: lavaTex || tex('lavaPuddle') || tex('emberWarm'), color: lavaTex ? 0xffffff : 0xff7a3a, opacity: 0.85 });
...
function _resetFires() {
  _fires.length = 0;
  if (!_inst) return;
  for (let i = 0; i < FIRE_CAP; i++) _hide(i);
...
export function tickSignalFires(dt) {
  ...
  for (let i = 0; i < FIRE_CAP; i++) {
    ...
    let cands = null;
    _queryPos.x = f.x; _queryPos.z = f.z;
    try { cands = queryRadiusInto(_queryPos, f.radius, _queryBuf); } catch (_) { cands = null; }
    if (!cands) continue;
