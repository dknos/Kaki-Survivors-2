...
export default {
  ...
  init(state, level, inst) {
    ...
    inst.innerAngle = 0;
    inst.outerAngle = 0;
    inst.lastHit = new WeakMap();  // enemyRef -> last-hit-time, without retaining dead enemies
  },
  ...
  dispose(state, inst) {
    if (_inst) {
      for (let i = 0; i < SAT_CAP; i++) _hide(i);
      _inst.instanceMatrix.needsUpdate = true;
    }
    inst.lastHit = new WeakMap();
  },
};

function _check(inst, x, z, dmg, interval, now) {
  let cands = null;
  _queryPos.x = x; _queryPos.z = z;
  try { cands = queryRadiusInto(_queryPos, HIT_RADIUS, _queryBuf); } catch (_) { cands = null; }
  if (!cands) return;
