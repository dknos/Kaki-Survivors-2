/**
 * Arc Grappler — Pipes' signature weapon.
 *
 * The automatic Arc Wrench line bolt remains Pipes' reliable survivor-style
 * baseline. His exclusive manual layer is the Rescue Hook: hold RMB/LT to grab
 * a mob, orbit it, shoot the captive to overcharge the cable, then release to
 * throw it through the horde. Q/RB remains the drafted Active ability.
 *
...
export default {
  ...
  avatar: 'pipes',
  name: 'Arc Grappler',
  desc: 'Auto pierce bolts. Hold RMB to grab a mob; shoot to charge, release to hurl.',
  icon: '🥸',
  maxLevel: 8,
  ...
  init(state, level, inst) {
    inst.cd = 0;
    inst.castCount = 0;
    initPipesGrapple(state, level, inst);
  },

  tick(state, dt, level, inst) {
    tickPipesGrapple(state, dt, level, inst);
  ...
  refresh(state, level, inst) {
    refreshPipesGrapple(state, level, inst);
    if (inst.cd > level.cooldown * 0.5) inst.cd = level.cooldown * 0.25;
  },

  dispose(state, inst) {
    disposePipesGrapple(state, inst);
  },
};
