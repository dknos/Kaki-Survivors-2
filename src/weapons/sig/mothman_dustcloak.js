...
function _writeCloudMatrix(i, c) {
  ...
  _scale.set(r, r, r);
  _m4.compose(_v3, _identityQuat, _scale);
...
function _resetClouds() {
  _clouds.length = 0;
  if (!_inst) return;
  for (let i = 0; i < CLOUD_CAP; i++) _hide(i);
...
export function tickDustClouds(dt) {
  ...
  for (let i = 0; i < CLOUD_CAP; i++) {
    ...
    let cands = null;
    _queryPos.x = c.x; _queryPos.z = c.z;
    try { cands = queryRadiusInto(_queryPos, c.radius, _queryBuf); } catch (_) { cands = null; }
    if (!cands) continue;
