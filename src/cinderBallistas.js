...
// Pre-allocated arrays for spatial queries to eliminate per-frame allocations
const _scanCands = [];
const _hitCands = [];
...
// Short traveling streak. The previous 30u mesh already covered the entire
// max range at spawn and then traveled another 30u while collision only swept
// its origin, producing a misleading screen-wide beam.
export const BOLT_LENGTH = 2.4;
...
const BOLT_POOL_SIZE = 12; // six turrets, 2s cadence, <0.5s flight lifetime
...
// ─── bolt projectile pool (slot 7, bloom ON) ─────────────────────────────────
// At most six turrets can fire together and every flight lasts under 0.5s, so
// twelve prebuilt streaks cover the worst case without per-shot GPU allocation.
function _buildBoltPool(parent) {
  const geo = new THREE.CylinderGeometry(BOLT_RADIUS, BOLT_RADIUS, BOLT_LENGTH, 8, 1, false);
  geo.rotateZ(-Math.PI / 2);          // Y-axis → +X axis
  // Pool position is the collision/leading point; the luminous streak trails
  // behind it, so neither damage nor art exceeds the 30u range.
  geo.translate(-BOLT_LENGTH * 0.5, 0, 0); // spans -length..0 along travel
  const mat = new THREE.MeshBasicMaterial({
  ...
  for (let i = 0; i < BOLT_POOL_SIZE; i++) {
    ...
    group.name = `cinderBallistaBolt_${i}`;
    group.visible = false;
    group.add(mesh);
    parent.add(group);
    _boltPool.push({
      active: false,
      x: 0, z: 0, vx: 0, vz: 0, spawnX: 0, spawnZ: 0, life: 0,
      hitSet: new Set(), group,
    });
  }
}

function _spawnBolt(ox, oz, dirX, dirZ) {
  let bolt = null;
  for (let i = 0; i < _boltPool.length; i++) {
    if (!_boltPool[i].active) { bolt = _boltPool[i]; break; }
  }
  if (!bolt) return null;
  bolt.active = true;
  bolt.x = bolt.spawnX = ox;
  bolt.z = bolt.spawnZ = oz;
  bolt.vx = dirX * BOLT_SPEED;
  bolt.vz = dirZ * BOLT_SPEED;
  bolt.life = BOLT_MAX_RANGE / BOLT_SPEED + 0.1;
  bolt.hitSet.clear();
  bolt.group.position.set(ox, 0.7, oz);
  bolt.group.rotation.y = Math.atan2(-dirZ, dirX);
  bolt.group.visible = true;
  return bolt;
}

/** Shared activation seam used by the 10-second repair and Forgeheart finds. */
function _activateBallista(b) {
  if (!b || b.activated || b.state === 'activated') return false;
  b.state = 'activated';
  b.activated = true;
  b.repairTimer = REPAIR_DURATION;
  ...
  if (b.repairAura) b.repairAura.mesh.visible = false;
  try { sfx.ballistaActivate && sfx.ballistaActivate(); } catch (_) {}
  return true;
}

/**
 * A dash-shattered Forgeheart awakens the closest dormant outpost. The range
 * is deliberately bounded so every core has an authored ballista partner and
 * cannot silently activate a turret on the far side of the map.
 */
export function activateNearestBallista(x, z, maxRange = 16) {
  let nearest = null;
  let nearestD2 = maxRange * maxRange;
  for (const b of _ballistas) {
    if (!b || b.activated || b.state === 'activated') continue;
    const dx = b.x - x;
    const dz = b.z - z;
  ...
  return _activateBallista(nearest);
}
...
export async function loadCinderBallistas(scene, hotspotsUrl = 'assets/cinder_ballista_hotspots.json') {
  ...
  // Batch the four fully static chassis layers. Only the aiming cradle and
  // per-ballista glow material remain individual so activation can animate.
  const baseInst = new THREE.InstancedMesh(baseGeo, baseMat, hotspots.length);
  const platformInst = new THREE.InstancedMesh(platformGeo, platformMat, hotspots.length);
  const rustInst = new THREE.InstancedMesh(rustGeo, rustMat, hotspots.length);
  const rackInst = new THREE.InstancedMesh(rackGeo, rackMat, hotspots.length);
  baseInst.name = 'cinderBallistaBases';
  platformInst.name = 'cinderBallistaPlatforms';
  rustInst.name = 'cinderBallistaBands';
  rackInst.name = 'cinderBallistaRacks';
  for (const inst of [baseInst, platformInst, rustInst, rackInst]) {
    inst.castShadow = false;
    inst.receiveShadow = true;
    _group.add(inst);
    _disposables.push(inst);
  }
  const staticDummy = new THREE.Object3D();
  ...
  for (let hi = 0; hi < hotspots.length; hi++) {
    ...
    // ── Base + platform + rack (static silhouette, instanced above) ──
    staticDummy.scale.setScalar(s);
    staticDummy.rotation.set(0, 0, 0);
    staticDummy.position.set(h.x, 0.225 * s, h.z);
    staticDummy.updateMatrix();
    baseInst.setMatrixAt(hi, staticDummy.matrix);
    staticDummy.position.y = 0.50 * s;
    staticDummy.updateMatrix();
    platformInst.setMatrixAt(hi, staticDummy.matrix);
    staticDummy.position.y = 0.55 * s;
    staticDummy.updateMatrix();
    rustInst.setMatrixAt(hi, staticDummy.matrix);
    rackInst.setMatrixAt(hi, staticDummy.matrix);
    ...
    _ballistas.push({
      ...
      repairAura: _spawnRepairAura(entGroup, 0, 0), // pooled; toggled visible
      rng,
    });
  }
  for (const inst of [baseInst, platformInst, rustInst, rackInst]) {
...
export function tickCinderBallistas(dt, state) {
  ...
  for (const b of _ballistas) {
      ...
      if (heroAlive && heroPos) {
        ...
        if (dx * dx + dz * dz <= PROXIMITY_R2) {
          ...
          if (b.repairAura) {
            ...
            b.repairAura.mesh.visible = true;
          }
    ...
    let scanCands = null;
    if (state.enemies && state.enemies.spatial) {
      try {
        scanCands = state.enemies.spatial.queryRadiusInto({ x: b.x, z: b.z }, BOLT_MAX_RANGE, _scanCands);
    ...
    if (b.fireTimer <= 0 && nearestE && nearestD2 <= BOLT_MAX_RANGE * BOLT_MAX_RANGE) {
      ...
      const bolt = _spawnBolt(b.x, b.z, dirX, dirZ);
      if (bolt) _activeBolts.push(bolt);
      b.fireTimer = BOLT_INTERVAL;
  ...
  for (let i = _activeBolts.length - 1; i >= 0; i--) {
    ...
    if (state.enemies && state.enemies.spatial) {
      try {
        hits = state.enemies.spatial.queryRadiusInto({ x: midX, z: midZ }, scanR, _hitCands);
      } catch (_) { hits = null; }
    }
    if (!hits) {
      const list = (state.enemies && state.enemies.active) ? state.enemies.active : null;
...
export function clearCinderBallistas(scene) {
  _loadGeneration++;
  // Reset pooled bolts. Their shared geometry/material live in _disposables.
