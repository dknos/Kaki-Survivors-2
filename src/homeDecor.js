...
// ── Grid (centered at room origin, 16 cols × 12 rows). The room is 20×15
// world-space; the grid uses the central 16×12 (±2.5u x-margin, ±1.5u
// z-margin for walls + perimeter fixtures). Tile pitch = 1u. ───────────────
export const TILE = 1.0;
export const GRID_COLS = 16;     // x axis
export const GRID_ROWS = 12;     // z axis
...
export function tileToWorld(gx, gz) {
  // Origin sits between tile (7.5, 5.5). Subtract half-grid to center.
...
// Big-cozy layout: every functional fixture is pushed to the room's perimeter
// (walls / corners) so the whole 16×12 central grid is open floor. Fixtures
// mostly sit OUTSIDE the grid extent (x beyond ±7.5, z beyond ±5.5); we only
// reserve the thin grid-edge strip each one's footprint can poke into. Fixture
// world coords (interior.js buildInterior/enterInterior — keep in sync):
//   Renovations Desk  back wall left   (-5.0, -6.2) → edge tiles (2,0)(3,0)(4,0)
//   Tea Kettle        back wall center  (0.0, -6.5) → edge tiles (7,0)(8,0)
//   Sketchbook Easel  back wall right  ( 5.0, -6.2) → edge tiles (12,0)(13,0)(14,0)
//   Computer Desk     SW corner        (-8.0,  5.8) → corner tiles (0,10)(0,11)(1,11)
//   Yarn Basket       SE corner        ( 8.3,  5.8) → corner tile (15,11)
//   Fireplace         west wall center (-9.6,  0.0) → outside grid, no reserve
//   Door / entry walk front wall center → tiles (7,11)(8,11)
const RESERVED_TILES = new Set([
  // Back wall — renovations desk / tea kettle / sketchbook easel
  '2,0','3,0','4,0',
  '7,0','8,0',
  '12,0','13,0','14,0',
  // SW corner — computer desk
  '0,10','0,11','1,11',
  // SE corner — yarn basket
  '15,11',
  // Front wall — door + entry walkway (don't box the player in on arrival)
  '7,11','8,11',
]);
...
export const HOME_CATALOG = [
  ...
  {
    id: 'banner_alt', name: 'Kiln-Shift Banner', icon: '🚩',
    glb: 'home_banner_alt', scale: 1.6, wall: true,
    unlock: 'Unlock Ceramic Basin.',
  ...
  },
  {
    ...
    unlock: 'Win Boss Rush in Paper Woods (unlock Clockwork).',
...
export function syncHomeUnlocks() {
  ...
  for (const item of HOME_CATALOG) {
    ...
    if (ok) {
      ...
      // Default (starter) items grant silently — they're the baseline kit, not
      // an earned reward, so they shouldn't fire the unlock-toast celebration.
      if (item.default) grantedSilently = true;
      else granted.push(item);
    }
  }
  if (granted.length || grantedSilently) saveMeta();
  return granted;
}
...
function _buildOverlay() {
  ...
  // Instruction strip. Lead with the actual placement flow (players didn't
  // realize you pick an item THEN click the floor), then the keyboard/mouse
  // controls. The gamepad line is shown ONLY when a pad is connected — a
  // mouse+keyboard player has no use for "D-PAD / A / B / X / Y" clutter.
  ...
  `;
  let html =
    '<span style="color:#ffd27f;">Pick an item, then click the floor to place.</span><br>' +
