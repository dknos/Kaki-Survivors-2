...
export const LORE = {
  ...
  goblin:      { name: 'Pocket Scrounger', icon: '🧤', text: 'Checks every unattended pocket twice. The second check is personal.' },
  skeleton:    { name: 'Buttonbones', icon: '🦴', text: 'Held together by coat buttons and professional patience. One is always fastened correctly.' },
  orc:         { name: 'Apron Bruiser', icon: '🧤', text: 'Clocked out years ago. The apron stayed for one more shift.' },
  demon:       { name: 'Kiln Grin', icon: '♨️', text: 'Fired too hot and pleased about it. Step aside before the glaze cracks.' },
  robot:       { name: 'Tin Clerk', icon: '🗃️', text: 'Still processing route forms. Yours has been marked urgent for no reason.' },
  mech:        { name: 'Overtime Frame', icon: '🦾', text: 'A work suit with nobody inside. It continues the shift out of habit.' },
  xeno:        { name: 'Wrong-Way Guest', icon: '↩️', text: 'Arrived by a road facing backward. Too polite to leave by another.' },
  slime:       { name: 'Inkwell Leak', icon: '◉', text: 'A spilled map note looking for a margin. Do not provide one.' },
  giant:       { name: 'Heavy Apron Weighscale', icon: '⚖️', text: 'Elite. Counts every footstep, then adds one. Nobody has corrected it.' },
  dragon:      { name: 'Weathered Wind Vane', icon: '↗️', text: 'Elite. Turns toward incoming trouble. Unfortunately, that is you.' },
  spider:      { name: 'Threadleg', icon: '🕷️', text: 'Eight loose stitches moving as one bad decision. Thin, quick, and difficult to apologize to.' },
  wolf:        { name: 'Night Route Hound', icon: '🐺', text: 'Follows the path after the path has gone home. The pack remembers every shortcut.' },
  wizard:      { name: 'Receipt Slinger', icon: '🧾', text: 'Keeps its distance and itemizes your mistakes. The total is always moving.' },
  ghost:       { name: 'Unclaimed Lantern', icon: '🏮', text: 'A light with nobody carrying it. Direct attention makes it briefly honest.' },
  clockwork_mouse: { name: 'Clockwork Mouse', icon: '🐭', text: 'A runaway daycare toy with too much spring in its key. Its paw-mark shows where the next pounce lands — move before it clicks.' },
  yarn_wisp:   { name: 'Yarn Wisp',     icon: '🧶', text: 'A grumpy knot of moon-thread that refuses to unravel. Its rotating five-strand volley turns open ground into a tiny weaving puzzle.' },
  dragon_evo:  { name: 'Moonwing',      icon: '🐲', text: 'An old dragon lacquered in night-sky scales. It hangs beyond claw range and folds five burning comets across your escape lane.' },
  kaki_sparkmite:  { name: 'Sparkmite',  icon: '🔥', text: 'A cinder kitten born from the Ember Warden\'s cracked crown. Its landing paw is a promise: leave before the spring snaps.' },
  kaki_tidesprite: { name: 'Tidesprite', icon: '🌊', text: 'A sky-current familiar that fights from the edge. Five slow currents turn open space into a weaving lane.' },
  kaki_bloomling:  { name: 'Bloomling',  icon: '🌸', text: 'A ward-seed with a bright pollen heart. Strip its short petal shield, then interrupt the three-shot choir.' },
...
export function notifyEvolutionAchieved(id) {
  ...
  // Maintain a durable "every evolution discovered" flag for the master_collector
  // achievement. meta.js's DAG sweep reads meta.codex._allEvosDiscovered — it
  // can't import EVOLUTIONS itself (weapons imports meta → circular), so compute
  // it here where EVOLUTIONS is reachable. Best-effort dynamic import.
  try {
    import('./weapons/index.js').then(({ EVOLUTIONS }) => {
      const meta = getMeta();
      if (!meta.codex || !meta.codex.evolutions || !EVOLUTIONS) return;
      const total = Object.keys(EVOLUTIONS).length;
      const have = Object.keys(meta.codex.evolutions).length;
      if (total > 0 && have >= total && !meta.codex._allEvosDiscovered) {
        meta.codex._allEvosDiscovered = true;
        saveMeta();
      }
    }).catch(() => {});
  } catch (_) {}
