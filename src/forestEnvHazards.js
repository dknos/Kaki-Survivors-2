...
import { FOREST_ROOMS, FOREST_PORTAL_POSITIONS, getForestTravelAnchors } from './forestRooms.js';
...
import { applyFloorTier, floorDecalGeometry } from './fxLayers.js';
...
const SLOT4_DARK   = 0x4a3220;   // tar fallback tint
...
// Portal interactions reach ~2.2u and the widest hazard reaches 2u. A 5u
// center keepout leaves a safety margin around both Glade gates and every
// side-room entry/return anchor, including low-health departure frames that
// run before portal activation can grant arrival i-frames.
const PORTAL_KEEPOUT_R    = 5;
const PORTAL_KEEPOUT_R2   = PORTAL_KEEPOUT_R * PORTAL_KEEPOUT_R;
const LANDMARK_KEEPOUT_R2 = 2 * 2;
const _travelAnchorKeepouts = [];
for (const roomId in FOREST_ROOMS) {
  const anchors = getForestTravelAnchors(roomId);
  if (!anchors) continue;
  _travelAnchorKeepouts.push(anchors.entry, anchors.return);
}
...
function _isInKeepout(x, z, landmarkPositions) {
  ...
  for (let i = 0; i < _travelAnchorKeepouts.length; i++) {
    const p = _travelAnchorKeepouts[i];
    dx = x - p.x;
    dz = z - p.z;
    if (dx * dx + dz * dz < PORTAL_KEEPOUT_R2) return true;
  }
...
function _buildTarPitMesh() {
  // Asset-backed, irregular paw-bog decal. The old featureless CircleGeometry
  // read as an accidental shadow; this silhouette communicates mud, danger,
  // and the cat theme before the player steps into its slow/DoT volume.
  const tarTex = fxTex('forest_tar_bog');
  const fallbackTex = tarTex || tex('smokeDark');
  const geo = fallbackTex
    ? floorDecalGeometry(2.0)
    : (() => { const g = new THREE.CircleGeometry(1.0, 28); g.rotateX(-Math.PI / 2); return g; })();
  const mat = new THREE.MeshBasicMaterial({
    map: fallbackTex,
    color: tarTex ? 0xffffff : SLOT4_DARK,
    transparent: true,
    opacity: tarTex ? 0.94 : 0.68,
    alphaTest: fallbackTex ? 0.025 : 0,
    depthTest: true,
    depthWrite: false,
  ...
  _tarMesh.userData.visualRole = 'terrain_hazard';
  _tarMesh.userData.gameplayPurpose = 'slow-and-damage-both-sides';
  applyFloorTier(_tarMesh, 'terrain_hazard', { bloom: false });
...
function _tickMushroomRings(state, dt) {
  ...
  const spatial = state && state.enemies && state.enemies.spatial;
  const activeFallback = (state && state.enemies && state.enemies.active) || null;
  ...
  for (let r = 0; r < _ringCount; r++) {
    ...
    if (phase === MP_IDLE) {
      ...
      let cands = activeFallback;
      if (spatial) {
        // dummy position object for queryRadiusInto
        _dummy.position.set(cx, 0, cz);
        cands = spatial.queryRadiusInto(_dummy.position, SPORE_PUFF_RADIUS, _mushroomNearby);
      }
...
function _tickTarPits(state, dt) {
  ...
  for (let i = 0; i < _tarCount; i++) {
    ...
    if (spatial) {
      _dummy.position.set(cx, 0, cz);
      cands = spatial.queryRadiusInto(_dummy.position, TARPIT_RADIUS, _tarNearby);
    }
...
function _tickBranchTriggers(state, dt) {
  ...
  for (let ti = 0; ti < _triggerCount; ti++) {
    ...
    _branchCount++;
    _branchBoxMesh.visible = true;
    _branchRingMesh.visible = true;
  ...
  for (let i = 0; i < CAP_ACTIVE_BRANCHES; i++) {
    ...
    if (phase === BP_ARMED) {
      ...
      if (spatial) {
        _dummy.position.set(cx, 0, cz);
        cands = spatial.queryRadiusInto(_dummy.position, BRANCH_DMG_RADIUS, _branchNearby);
      }
...
/** Focused QA snapshot for portal-safety regression tests. */
export function _debugForestEnvHazardPlacements() {
  const unpack = (buf, count) => {
    const out = [];
    if (!buf) return out;
    for (let i = 0; i < count; i++) out.push({ x: buf[i * 2], z: buf[i * 2 + 1] });
    return out;
  };
  return {
    portalKeepoutRadius: PORTAL_KEEPOUT_R,
    travelAnchors: _travelAnchorKeepouts.map((p) => ({ x: p.x, z: p.z })),
    mushroomRings: unpack(_ringPos, _ringCount),
    tarPits: unpack(_tarPos, _tarCount),
    branchTriggers: unpack(_triggerPos, _triggerCount),
  };
}
