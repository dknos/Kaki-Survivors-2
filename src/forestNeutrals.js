 ...
 *
 * Two NON-COMBAT entity types that ship visual flavor only. No XP drops,
 ...
 *   tickForestNeutrals(state, dt) — per-frame: drift fireflies and advance
 *     deer state machines. Cheap when not loaded.
 ...
 *   slot 4 #4a3220 — dark brown (deer legs)
 *   slot 6 #d9a648 — gold/amber (firefly dot)
 *
 * Constraints honored:
 ...
 *   - Bloom layer: ONLY firefly dots. Deer bodies stay off-bloom so a
...
const SLOT4_DARK  = 0x4a3220; // deer legs
const SLOT6_GOLD  = 0xd9a648; // firefly dot
...
// do not need the movement keep-out used by deer.
...
/**
 * Per-frame: drift fireflies and advance deer state machines.
...
export function tickForestNeutrals(state, dt) {
  ...
  // forestLandmarks observes: deer don't flee a dead hero; fireflies keep
  // drifting because they are decor only.
