 ...
 *
 * Roll weights by D(t):
 *   D < 0.8        → no affixes (tutorial-friendly opening)
 *   0.8 ≤ D < 2    → Leaping ONLY, 2%→8% scale-in (telegraphed jumps in min 1-2)
...
/** Apply one authored affix without rolling; used by enemies whose mechanic
 * is part of their permanent identity. Duplicate IDs are ignored so a random
 * roll can never double-stamp a fixed behavior. */
export function applyAffix(enemy, id) {
  const affix = _affixById[id];
  if (!enemy || !affix) return false;
  if (enemy.affixes && enemy.affixes.includes(id)) return false;
  _applyAndStamp(enemy, affix);
  return true;
}
...
export function rollAffixes(enemy, D) {
  ...
  if (D < 2) {
    // Iter 2 — Leaping previews early (D ≥ 0.8, ~minute 1) so the player SEES
    // telegraphed jumps while the rest of the affix table waits for D=2. Leap
    // arcs are existing content (0.6s windup + landing marker = dodgeable).
    // Chance scales 2% → 8% across D 0.8→2 — conservative on purpose.
    if (D >= 0.8 && Math.random() < 0.02 + 0.06 * (D - 0.8) / 1.2) {
      _applyAndStamp(enemy, _affixById['leaping']);
    }
    return;
  }
