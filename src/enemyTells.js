 ...
 *
 * Nine pooled InstancedMesh layers, all on BLOOM_LAYER so they participate in the
 ...
 *
 * PERF: zero per-frame allocations — all temps are module-scope. Dynamic
 * families write compact live prefixes and submit count=0 while empty; sparse
 * leap markers retain stable slots with a bounded high-water count. Caps are
 * intentionally tight; if a horde overruns them the excess simply doesn't
 * render its tell (gameplay-safe).
 */
...
// Dynamic tell families write densely from slot zero every frame. Keep their
// submitted instance count at the live prefix instead of collapsing the unused
// capacity to zero-scale matrices. Besides skipping empty draws, this avoids
// uploading buffers for families that have no visible members.
function _commitCompactPool(inst, used, updateColor = false) {
  inst.count = used;
  if (used === 0) return;
  inst.instanceMatrix.needsUpdate = true;
  if (updateColor && inst.instanceColor) inst.instanceColor.needsUpdate = true;
}

function _leapHighWaterCount() {
  for (let i = LEAP_MARKER_CAP - 1; i >= 0; i--) {
    if (_leapSlots[i].used) return i + 1;
  }
  return 0;
}
...
export function setLeapMarker(x, z, remaining, total, key) {
  ...
  if (_leapMarkers.count <= slot) _leapMarkers.count = slot + 1;
  _leapMarkers.instanceMatrix.needsUpdate = true;
}
...
export function clearLeapMarker(enemyId) {
  ...
  for (let i = 0; i < LEAP_MARKER_CAP; i++) {
    ...
    if (s.key === enemyId) {
      ...
      s.key  = null;
      _leapMarkers.setMatrixAt(i, _hideMat);
      const nextCount = _leapHighWaterCount();
      _leapMarkers.count = nextCount;
      if (i < nextCount) _leapMarkers.instanceMatrix.needsUpdate = true;
      return;
    }
  }
}
...
// Per-frame update
// ──────────────────────────────────────────────────────────────────────────
export function updateEnemyTells(_dt) {
  if (!_ringsElite || !_rangedTells || !_threatDots) return;

  const t      = state.time.game;
  const heroP  = state.hero.pos;

  let active = state.enemies.active;
  if (heroP && state.enemies.spatial) {
    active = state.enemies.spatial.queryRadiusInto(heroP, 24, _nearby);
  }
  ...
  for (let i = 0; i < active.length; i++) {
    ...
    // Stable per-enemy animation phase. The loop index `i` can't be used here:
    // the spatial query reorders `active` every frame, so `i` would jump and the
    // ring/dot animations would snap. mesh.id is unique and fixed per enemy.
    const _phase = e.mesh.id;

    if (heroP && (!state.enemies.spatial || active === state.enemies.active)) {
      const dx = ep.x - heroP.x;
      const dz = ep.z - heroP.z;
    ...
    if (e.elite || wantsAffixRing || e.isMiniBoss || e.isFinalBoss) {
      ...
      const ringY = e._frostAura ? RING_Y + 0.04 + 0.025 * Math.sin(t * 2.2 + _phase) : RING_Y;
      _pos.set(ep.x, ringY, ep.z);
      _scl.set(ringPulse, 1, ringPulse);
      // Slow yaw rotation — sells "magic glyph rotating" vs flat decal.
      _quat.setFromAxisAngle(_axisY, t * 0.4 + _phase * 0.7);
      _mat.compose(_pos, _quat, _scl);
    ...
    if ((e.isMiniBoss || e.isFinalBoss || e._vampPct) && dotSlot < THREAT_DOT_CAP) {
      ...
      _pos.set(ep.x, DOT_Y + Math.sin(t * 3 + _phase) * 0.08, ep.z);
  ...
  // Every pool above writes a compact live prefix. Restrict submission to that
  // prefix instead of rewriting and uploading the unused capacity each frame.
  ...
  if (_leapMarkers) {
    let leapDirty = false;
    let firstCleared = LEAP_MARKER_CAP;
    for (let i = 0; i < LEAP_MARKER_CAP; i++) {
      const s = _leapSlots[i];
      if (s.used && s.touchedFrame !== _leapFrame) {
        ...
        _leapMarkers.setMatrixAt(i, _hideMat);
        leapDirty = true;
        if (i < firstCleared) firstCleared = i;
      }
    }
    if (leapDirty) {
      const nextCount = _leapHighWaterCount();
      _leapMarkers.count = nextCount;
      if (firstCleared < nextCount) _leapMarkers.instanceMatrix.needsUpdate = true;
    }
  }
...
export function resetEnemyTells() {
  if (!_ringsElite || !_rangedTells || !_threatDots) return;
  _ringsElite.count = 0;
  _ringsVolatile.count = 0;
  _ringsFrosted.count = 0;
  _ringsShielded.count = 0;
  _ringsMini.count = 0;
  _ringsFinal.count = 0;
  _rangedTells.count = 0;
  _threatDots.count = 0;
