 ...
 *   - paw-and-whisker reward marker overhead (non-ring, non-bloom)
 *
 ...
 *   slot 6  #d9a648 — lock + opening burst (amber)
 *
 * ── Hard caps ──────────────────────────────────────────────────────────────
...
function _buildChestMeshes() {
  ...
  // Reward marker: a compact paw badge, not a spinning gold circle. The
  // chest's lock and wood silhouette already carry its treasure identity.
...
// Stamp a chest's static meshes (body, bands, lock, marker) at world pos.
...
function _syncDrawCount() {
  ...
  const meshes = [
    _bodyMesh, _lidMesh, _band0Mesh, _band1Mesh, _band2Mesh, _band3Mesh,
    _lockMesh, _sparkleMesh, _burstMesh,
  ];
...
export function dropForestChest(pos) {
  ...
  const x = constrainForestX((typeof pos.x === 'number') ? pos.x : 0, 2);
  const z = constrainForestZ((typeof pos.z === 'number') ? pos.z : 0, 2);
  _phase[i]   = CP_CLOSED;
  _phaseT[i]  = 0;
  _posX[i]    = x;
  _posZ[i]    = z;
...
export function tickForestChests(state, dt) {
  ...
  // Breathe reward markers + pickup detect + phase advance.
  ...
  for (let i = 0; i < CAP_CHESTS; i++) {
    ...
    // Reward badge breathes while chest is visible; it intentionally does
    // not spin into a gold rune / target marker.
    if (phase === CP_CLOSED || phase === CP_OPENING) {
      const yaw = i * 0.32 + Math.sin(_clock * 1.2 + i) * 0.08;
      const bob = SPARKLE_Y + Math.sin(_clock * 2.0 + i) * 0.06;
      _dummy.position.set(cx, bob, cz);
      _dummy.rotation.set(0, yaw, 0);
      const pulse = 1 + Math.sin(_clock * 2.4 + i) * 0.05;
...
function _pickRandomEquippedWeapon() {
  // "Chests level YOUR build": pick the LOWEST-level below-cap equipped
  // weapon (skipping hidden slots like the auto-ranked primary) so chest
  // weapon rewards accelerate the build instead of padding whatever slot a
  // shuffle lands on. Forest is the default stage — this is the chest path
  // players actually hit; mirrors slotMachine's resolveWeaponTarget policy.
  if (!_gameState.weapons || _gameState.weapons.length === 0) return null;
  let best = null;
  for (let k = 0; k < _gameState.weapons.length; k++) {
    const w = _gameState.weapons[k];
    const mod = REGISTRY[w.id];
    if (mod && mod.hidden) continue;
    const cap = (mod && typeof mod.maxLevel === 'number') ? mod.maxLevel : WEAPON_CAP;
    if (w.level >= cap) continue;
