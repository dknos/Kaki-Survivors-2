...
let _enabled = true;
let _racingAudio = null;  // persistent engine + tire layer (created on demand)
let _racingNoise = null;  // shared deterministic buffer for tires + impacts
...
function _racingNoiseBuffer(ctx) {
  if (_racingNoise) return _racingNoise;
  const length = Math.max(1, Math.round(ctx.sampleRate * 0.75));
  const buffer = ctx.createBuffer(1, length, ctx.sampleRate);
  const data = buffer.getChannelData(0);
  let seed = 0x4b414b49;
  for (let i = 0; i < length; i++) {
    // Deterministic xorshift noise avoids a different harsh DC character each
    // time the player re-enters Rally while remaining cheap enough to build once.
...
function _ensureRacingAudio() {
  if (!_enabled) return null;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return null;
  if (_racingAudio) return _racingAudio;
  ...
  engine.connect(engineGain).connect(engineFilter);
  harmonic.connect(harmonicGain).connect(engineFilter);
  engineFilter.connect(mixGain).connect(_sfxBus);

  const tire = ctx.createBufferSource();
  const tireFilter = ctx.createBiquadFilter();
  const tireGain = ctx.createGain();
  tire.buffer = _racingNoiseBuffer(ctx);
  tire.loop = true;
  tireFilter.type = 'bandpass';
  tireFilter.frequency.value = 760;
  tireFilter.Q.value = 0.72;
  tireGain.gain.value = 0.0001;
  tire.connect(tireFilter).connect(tireGain).connect(_sfxBus);

  engine.start();
  harmonic.start();
  tire.start();
  _racingAudio = {
    ctx, engine, engineGain, harmonic, harmonicGain, engineFilter, mixGain,
    tire, tireFilter, tireGain, lastGear: 1, shiftDip: 0,
  };
  return _racingAudio;
}

/**
 * Smoothly update Rally's persistent engine and tire layer.
 *
 * This is intentionally safe to call every frame. Nodes are allocated only on
 * first use and all changes use AudioParam smoothing to avoid zipper noise.
 */
export function updateRacingAudio({
  ...
  const audio = _ensureRacingAudio();
  ...
  const load = monster
    ? Math.min(1, Math.max(throttleLoad * 0.62, Number(engineLoad) || 0))
    : throttleLoad;
  ...
  audio.engineFilter.frequency.setTargetAtTime(
    (monster ? 360 : 520) + speedRatio * 980 + load * 300 + (boost ? 1150 : 0) + heat * 220,
    now,
    0.06,
  );
  audio.mixGain.gain.setTargetAtTime(
...
/** Stop and release Rally's persistent audio nodes. Safe to call repeatedly. */
export function stopRacingAudio({ immediate = false } = {}) {
  const audio = _racingAudio;
  if (!audio) return false;
  _racingAudio = null;
  const now = audio.ctx.currentTime;
  const release = immediate ? 0.01 : 0.16;
  for (const gain of [audio.mixGain.gain, audio.tireGain.gain]) {
    try {
  ...
  for (const source of [audio.engine, audio.harmonic, audio.tire]) {
    try { source.stop(now + release + 0.02); } catch (_) {}
  }
  setTimeout(() => {
    for (const node of [
      audio.engine, audio.engineGain, audio.harmonic, audio.harmonicGain,
      audio.engineFilter, audio.mixGain, audio.tire, audio.tireFilter, audio.tireGain,
    ]) {
...
/**
 * Procedural transient layered under sampled Rally hits.
 * `kind` may be `landing`, `crash`, or `smash`.
 */
export function playRacingImpact({ strength = 0.5, kind = 'crash' } = {}) {
  if (!_enabled) return false;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return false;
  const amount = Math.min(1, Math.max(0, Number(strength) || 0));
  if (amount <= 0.001) return false;
  _playCounts.sfx++;
  const now = ctx.currentTime;
  const duration = 0.08 + amount * (kind === 'smash' ? 0.24 : 0.14);
  const noise = ctx.createBufferSource();
  const filter = ctx.createBiquadFilter();
  const noiseGain = ctx.createGain();
  noise.buffer = _racingNoiseBuffer(ctx);
  filter.type = kind === 'landing' ? 'lowpass' : 'bandpass';
  filter.frequency.value = kind === 'smash' ? 1050 : kind === 'landing' ? 360 : 720;
  filter.Q.value = kind === 'smash' ? 0.62 : 0.9;
  noiseGain.gain.setValueAtTime(0.018 + amount * 0.09, now);
  noiseGain.gain.exponentialRampToValueAtTime(0.0001, now + duration);
  noise.connect(filter).connect(noiseGain).connect(_sfxBus);

  const thump = ctx.createOscillator();
  const thumpGain = ctx.createGain();
  thump.type = 'sine';
  const startFrequency = kind === 'smash' ? 96 : kind === 'landing' ? 72 : 118;
  thump.frequency.setValueAtTime(startFrequency, now);
  thump.frequency.exponentialRampToValueAtTime(34, now + duration);
  thumpGain.gain.setValueAtTime(0.015 + amount * (kind === 'smash' ? 0.13 : 0.08), now);
  thumpGain.gain.exponentialRampToValueAtTime(0.0001, now + duration);
  thump.connect(thumpGain).connect(_sfxBus);
  noise.start(now);
...
// ── Primary fire zap ─────────────────────────────────────────────────────────
// Procedural (no sample bank) so the held-fire primary reads distinctly from
// the sampled weaponAutoaim it used to share. Unlike the legacy tone/sweep
// helpers above this routes through _sfxBus (it's combat SFX, not music) and
// bumps the sfx play counter so the P4G #141 mute=0-amplitude contract holds
// for procedural paths too.
function _playPrimaryZap(row) {
  if (!_enabled) return;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return;
  ...
  const g = ctx.createGain();
  osc.type = 'square';
  // ±4% start-pitch jitter — same anti-robotic trick the sample bank uses.
  // Start pitch drops ~60Hz per primary rank (1..8) so rank-ups are audible:
  // the report gets meatier as the weapon grows.
  const start = 1800 - 60 * (Math.max(1, Math.min(8, row || 1)) - 1);
  osc.frequency.setValueAtTime(start * (1 + (Math.random() - 0.5) * 0.08), t);
  osc.frequency.exponentialRampToValueAtTime(700, t + 0.05);
  g.gain.setValueAtTime(0, t);
  // Chatter band (0.12-0.20) — fires up to ~3.3/s while held.
...
// ── Nova Burst ───────────────────────────────────────────────────────────────
// Dedicated low moon-bloom impact plus a short glassy rise. Nova previously
// reused the dash sample, so the game's largest player-triggered blast sounded
// like movement. Two tiny procedural voices keep the effect unique without a
// new decode/download and still route through the normal SFX bus.
function _playNovaBurst(opts) {
  if (!_enabled) return;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return;
  const t = ctx.currentTime;
  const rank = Math.max(1, Math.min(5, (opts && opts.rank) || 1));
  const absorbed = Math.max(0, Math.min(12, (opts && opts.cleared) || 0));

  const thump = ctx.createOscillator();
  const thumpGain = ctx.createGain();
  thump.type = 'sine';
  thump.frequency.setValueAtTime(150 + absorbed * 2, t);
  thump.frequency.exponentialRampToValueAtTime(52, t + 0.28);
  thumpGain.gain.setValueAtTime(0, t);
  thumpGain.gain.linearRampToValueAtTime(0.34, t + 0.008);
  thumpGain.gain.exponentialRampToValueAtTime(0.0001, t + 0.34);
  thump.connect(thumpGain).connect(_sfxBus);

  const shimmer = ctx.createOscillator();
  const shimmerGain = ctx.createGain();
  shimmer.type = 'triangle';
  const start = 420 + rank * 36;
  shimmer.frequency.setValueAtTime(start, t);
  shimmer.frequency.exponentialRampToValueAtTime(start * 2.4, t + 0.13);
  shimmerGain.gain.setValueAtTime(0, t);
  shimmerGain.gain.linearRampToValueAtTime(0.19, t + 0.018);
  shimmerGain.gain.exponentialRampToValueAtTime(0.0001, t + 0.30);
  shimmer.connect(shimmerGain).connect(_sfxBus);

  _playCounts.sfx++;
  thump.start(t); thump.stop(t + 0.36);
  shimmer.start(t); shimmer.stop(t + 0.32);
}

// ── Pipes Arc Grappler ──────────────────────────────────────────────────────
// Four compact procedural cues make the full input phrase readable without a
// downloaded voice/sample pack: cable launch, magnetic latch, charged throw,
// and mob-on-mob impact. They share the normal SFX bus and play counter.
function _playGrappleCue(kind, opts) {
  if (!_enabled) return;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return;
  const t = ctx.currentTime;
  const charge = Math.max(0, Math.min(1, (opts && opts.charge) || 0));
  const specs = {
    fire:   { type: 'sawtooth', a: 980, b: 260, dur: 0.13, gain: 0.18 },
    latch:  { type: 'square',   a: 320, b: 920, dur: 0.10, gain: 0.22 },
    throw:  { type: 'sawtooth', a: 520 + charge * 260, b: 120, dur: 0.22, gain: 0.25 + charge * 0.08 },
    impact: { type: 'triangle', a: 190 + charge * 80, b: 54, dur: 0.18, gain: 0.30 + charge * 0.10 },
  };
  const spec = specs[kind] || specs.fire;
  const osc = ctx.createOscillator();
  const gain = ctx.createGain();
  osc.type = spec.type;
  osc.frequency.setValueAtTime(spec.a, t);
  osc.frequency.exponentialRampToValueAtTime(Math.max(1, spec.b), t + spec.dur);
  gain.gain.setValueAtTime(0, t);
  gain.gain.linearRampToValueAtTime(spec.gain, t + 0.004);
  gain.gain.exponentialRampToValueAtTime(0.0001, t + spec.dur);
  osc.connect(gain).connect(_sfxBus);

  // Impact gets one short inharmonic metal overtone. This is the payoff beat,
  // but stays below bomb/boss loudness even at full charge.
  let overtone = null;
  if (kind === 'impact' || kind === 'latch') {
    overtone = ctx.createOscillator();
    const og = ctx.createGain();
    overtone.type = 'sine';
    overtone.frequency.setValueAtTime(kind === 'impact' ? 1120 : 1680, t);
    overtone.frequency.exponentialRampToValueAtTime(kind === 'impact' ? 620 : 980, t + spec.dur * 0.72);
    og.gain.setValueAtTime(spec.gain * 0.42, t);
    og.gain.exponentialRampToValueAtTime(0.0001, t + spec.dur * 0.82);
    overtone.connect(og).connect(_sfxBus);
    overtone.start(t);
    overtone.stop(t + spec.dur + 0.02);
  }
...
// ── Perfect dodge chime ──────────────────────────────────────────────────────
// Rising two-note triangle chirp — deliberately unlike heroHurt (you did
// something RIGHT) and unlike starPickup (which fires constantly during loot
// vacuums). Procedural via _sfxBus, same contract notes as _playPrimaryZap.
function _playPerfectDodge() {
  if (!_enabled) return;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return;
  const t = ctx.currentTime;
  ...
  osc.frequency.exponentialRampToValueAtTime(1240, t + 0.07);
  osc.frequency.setValueAtTime(1240, t + 0.09);
  osc.frequency.exponentialRampToValueAtTime(1860, t + 0.16);
  g.gain.setValueAtTime(0, t);
...
// ── Cursed Bell toll ──────────────────────────────────────────────────────────
// Ominous low bell for src/bells.js — struck on each ring as an AUDIBLE enrage
// telegraph (the ring was previously silent, so the enrage pulse had no warning
// cue). Inharmonic partials give a metallic bell timbre; a slight downward bend
// reads "cursed" rather than "church bell". Procedural via _sfxBus, same P4G
// #141 counter contract as the other procedural SFX.
function _playBellToll() {
  if (!_enabled) return;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return;
  const t = ctx.currentTime;
  const base = 185; // dark G3-ish fundamental
  // Fundamental + two inharmonic partials (bell-like, not a pure tone).
  const partials = [
    { r: 1.0,  v: 0.34, d: 1.1 },
    { r: 2.02, v: 0.17, d: 0.8 },
    { r: 2.75, v: 0.10, d: 0.55 },
  ];
  for (const p of partials) {
    ...
    osc.frequency.exponentialRampToValueAtTime(f0 * 0.985, t + p.d); // cursed sag
    g.gain.setValueAtTime(0, t);
    g.gain.linearRampToValueAtTime(p.v, t + 0.006);                  // sharp strike
...
// ── Portal-shard collect chime ────────────────────────────────────────────────
// Bright ascending triad for src/portalShards.js — collecting a shard was FX +
// banner but silent. Deliberately unlike starPickup (which fires constantly in
// loot vacuums): this reads "you got a rare objective piece". Procedural.
function _playShardChime() {
  if (!_enabled) return;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return;
  const t = ctx.currentTime;
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    osc.type = 'triangle';
    const ts = t + i * 0.05;
    osc.frequency.setValueAtTime(notes[i], ts);
    g.gain.setValueAtTime(0, ts);
    g.gain.linearRampToValueAtTime(0.24, ts + 0.008);
...
// ── Crit ping ─────────────────────────────────────────────────────────────────
// Short bright "ching" accent layered ON TOP of the hit sample for critical
// hits (crits previously reused sfx.hit with no distinct audio, so they were
// invisible to the ear). High, quick, low-gain — accents without dominating the
// combat mix. Gated to discrete weapon hits by the caller (not tick sources).
function _playCritPing() {
  if (!_enabled) return;
  const ctx = ensureCtx();
  if (ctx.state !== 'running') return;
  const t = ctx.currentTime;
  ...
  const g = ctx.createGain();
  osc.type = 'triangle';
  osc.frequency.setValueAtTime(2600 * (1 + (Math.random() - 0.5) * 0.06), t);
...
// ── Kill-streak ladder (iteration 1 feel pass) ──────────────────────────────
// enemyDeath pitches upward as kills land inside a rolling 1.5s window, with a
// slight gain ramp. Timestamps are counted INSIDE the throttle so only audible
// plays grow the streak; caller-threaded rate (dmg-scaled) multiplies on top.
// The streak RESETS when the hero takes a hit (see sfx.heroHurt) — the climb
// reads as "flawless slaughter since last mistake" instead of pinning at max
// pitch for entire farm phases, where the reward signal became the baseline.
const _deathTimes = [];
function _deathStreakOpts(o) {
  const now = _nowMs();
  while (_deathTimes.length && now - _deathTimes[0] > 1500) _deathTimes.shift();
  const streak = Math.min(12, _deathTimes.length);
  _deathTimes.push(now);
  const out = { gain: 0.40, ...(o || {}) };
  out.rate = (out.rate != null ? out.rate : 1) * (1 + 0.04 * streak);
  out.gain = Math.min(0.55, out.gain + 0.01 * streak);
  return out;
}

// ── Gem-vacuum combo (iteration 1 feel pass) ────────────────────────────────
// Pickup pitch ramps 1.0 → 1.8 while gems land <250ms apart; a 400ms gap
// resets. Gaps in between hold the current rate so near-misses don't whiplash.
let _pickupComboRate = 1.0;
let _pickupLastAt = 0;
function _pickupComboOpts() {
  const now = _nowMs();
  const gap = now - _pickupLastAt;
  _pickupLastAt = now;
  if (gap > 400) _pickupComboRate = 1.0;
  else if (gap < 250) _pickupComboRate = Math.min(1.8, _pickupComboRate + 0.05);
  return { gain: 0.26, rate: _pickupComboRate };
}
...
export const sfx = {
  ...
  pickup:   _throttled('pickup',   () => _play('pickup',   _pickupComboOpts())),
  ...
  // ── Weapon hooks ───────────────────────────────────────────────────────────
  // weaponPrimary + perfectDodge are procedural hooks (no sample bank).
  weaponPrimary: _throttled('weaponPrimary', (o) => _playPrimaryZap(o && o.row)),
  weaponNova:    _throttled('weaponNova',    (o) => _playNovaBurst(o)),
  grappleFire:   _throttled('grappleFire',   () => _playGrappleCue('fire')),
  grappleLatch:  _throttled('grappleLatch',  () => _playGrappleCue('latch')),
  grappleThrow:  _throttled('grappleThrow',  (o) => _playGrappleCue('throw', o)),
  grappleImpact: _throttled('grappleImpact', (o) => _playGrappleCue('impact', o)),
  perfectDodge:  _throttled('perfectDodge',  () => _playPerfectDodge()),
  ...
  enemyDeath: _throttled('enemyDeath', (o) => _play('enemyDeath', _deathStreakOpts(o))),
  eliteDeath: _throttled('eliteDeath', (o) => _play('eliteDeath', { gain: 0.55, ...(o || {}) })),

  // ── Hero reactions ─────────────────────────────────────────────────────────
  heroHurt:  _throttled('heroHurt',  (o) => { _deathTimes.length = 0; _play('heroHurt',  { gain: 0.50, ...(o || {}) }); }),
  ...
  // Procedural (no sample bank) — see the _play* helpers above.
  bellToll:      _throttled('bellToll',      () => _playBellToll()),      // Cursed Bell ring
  shardCollect:  _throttled('shardCollect',  () => _playShardChime()),    // portal-shard pickup
  // Crit accent. Own 90ms debounce (not the shared 30ms) so it fires from ANY
  // source — including the always-on starter orbitals — yet a dense swarm's
  // crits read as a flurry of chings rather than a solid tone (~11/s cap).
  crit:       (() => {
    let _lastCrit = 0;
    const CRIT_DEBOUNCE_MS = 90;
    return () => {
      const now = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
      if (now - _lastCrit < CRIT_DEBOUNCE_MS) return;
      _lastCrit = now;
      _playCritPing();
    };
  })(),
