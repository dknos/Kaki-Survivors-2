 ...
 *
 *   • Bomb     — 300 dmg to enemies within 16 units (excluding final-boss /
 *                Reaper / `_invincible`; elites/minibosses capped at 50% of
 *                their max HP). White screen flash + shake. Drops 0.35% of
 *                any enemy kill (forest only).
 ...
 *   random() roll allocates ranges 0–0.0035 (bomb), 0.0035–0.03 (magnet),
...
// Use the original marker's proven hover plane: Forest stacks authored ground
// and bridge depth above y=.5 in places. The compact paw remains depth-tested
// and is collected before the hero can overlap it, so it stays a world-space
// pickup cue instead of painting over actors like HUD FX.
const PICKUP_AURA_Y    = 0.95;
const PICKUP_AURA_SIZE = 1.18;
...
// Bomb effect
const BOMB_DMG         = 240;
const BOMB_RADIUS      = 16;   // on-screen blast only — spares the offscreen top-up buffer
...
function _buildBombMeshes() {
  ...
  _bombBodyMesh.castShadow = false;
...
function _buildMagnetMeshes() {
  ...
  _magProngLMesh.castShadow = false;
  _magProngRMesh.castShadow = false;
...
function _buildChickenMeshes() {
  ...
  _chickBodyMesh.castShadow = false;
...
function _buildSparkleMesh(cap) {
  // Authored pastel paw badge. The old generic gold rune ring looked like an
  // empty objective marker whenever the tiny pickup was occluded. This
  // purpose-built broken-whisker silhouette sits below actors and opts out of
  // selective bloom so it stays crisp instead of collapsing into a yellow O.
  const geo = new THREE.PlaneGeometry(PICKUP_AURA_SIZE, PICKUP_AURA_SIZE);
  geo.rotateX(-Math.PI / 2);
  const mat = new THREE.MeshBasicMaterial({
    map: fxTex('pickup_paw_aura'),
    color: 0xffffff,
    transparent: true,
    opacity: 0.78,
    depthTest: true,
  ...
  mesh.userData.pickupPart = 'aura';
  mesh.userData.asset = 'pickup_paw_aura';
  mesh.userData.visualRole = 'interactive_pickup_aura';
  mesh.userData.gameplayPurpose = 'walk-over consumable';
...
function _setDrawCount(meshes, count) {
  for (let i = 0; i < meshes.length; i++) {
    if (meshes[i]) meshes[i].count = count;
  }
}

function _trimDrawCount(phase, meshes, current) {
  let next = current;
  while (next > 0 && phase[next - 1] === CP_FREE) next--;
  if (next !== current) _setDrawCount(meshes, next);
  return next;
}

// Stable arrays: never allocate mesh lists in the per-frame pickup tick.
const _bombDrawMeshes = [];
const _magnetDrawMeshes = [];
const _chickenDrawMeshes = [];
let _bDrawCount = 0;
let _mDrawCount = 0;
let _cDrawCount = 0;
...
function _stampSparkle(mesh, i, x, z, scale) {
  ...
  const pulse = 0.90 + Math.sin(_clock * 2.2 + i * 1.2) * 0.08;
  _dummy.position.set(x, PICKUP_AURA_Y, z);
  _dummy.rotation.set(0, yaw, 0);
  _dummy.scale.setScalar(scale * pulse);
  _dummy.updateMatrix();
...
function _fireBombEffect(cx, cz) {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    ...
    const p = e.mesh && e.mesh.position;
    if (!p) continue;
    const dx = p.x - cx, dz = p.z - cz;
    if (dx * dx + dz * dz > BOMB_RADIUS * BOMB_RADIUS) continue;
    // Elites/minibosses can't be deleted by a bomb — cap at half their max HP.
    const dmg = (e.elite || e.isMiniBoss)
      ? Math.min(BOMB_DMG, (e.hpMax || BOMB_DMG) * 0.5)
      : BOMB_DMG;
    try { damageEnemy(e, dmg, 'bomb_pickup'); }
    catch (err) { /* swallow — bomb dispatch must never throw mid-loop */ }
  }
  try { burstExplosion(cx, cz, BOMB_RADIUS, SLOT6_GOLD); } catch (_) {}
...
function _showFlash() {
  ...
  if (!el) return;
  // 0.4 not 0.85: the bomb is a radius-16 blast now, not a screen wipe —
  // a full whiteout promised "everything dies" and read as a bug when
  // enemies outside the radius walked out of it.
...
function _spawnBomb(x, z) {
  ...
  if (i >= _bDrawCount) {
    _bDrawCount = i + 1;
    _setDrawCount(_bombDrawMeshes, _bDrawCount);
  }
...
function _spawnMagnet(x, z) {
  ...
  if (i >= _mDrawCount) {
    _mDrawCount = i + 1;
    _setDrawCount(_magnetDrawMeshes, _mDrawCount);
  }
...
function _spawnChicken(x, z) {
  ...
  if (i >= _cDrawCount) {
    _cDrawCount = i + 1;
    _setDrawCount(_chickenDrawMeshes, _cDrawCount);
  }
 ...
 *   [0.0000, 0.0035) → bomb     (0.35%)
 *   [0.0035, 0.03)   → magnet   (~2.65%)
...
export function dropForestPickup(pos, rngRoll, heroHpPct) {
  ...
  const x = constrainForestX((typeof pos.x === 'number') ? pos.x : 0, 1);
  const z = constrainForestZ((typeof pos.z === 'number') ? pos.z : 0, 1);
  if (rngRoll < 0.0035) {
...
export function tickForestPickups(state, dt) {
  ...
  _bDrawCount = _trimDrawCount(_bPhase, _bombDrawMeshes, _bDrawCount);
  ...
  _mDrawCount = _trimDrawCount(_mPhase, _magnetDrawMeshes, _mDrawCount);
  ...
  _cDrawCount = _trimDrawCount(_cPhase, _chickenDrawMeshes, _cDrawCount);
...
export function loadForestPickups(scene, _state) {
  ...
  _bombDrawMeshes.push(_bombBodyMesh, _bombFuseMesh, _bombSparkMesh, _bombSparkleMesh);
  _magnetDrawMeshes.push(_magProngLMesh, _magProngRMesh, _magTopMesh, _magSparkleMesh);
  _chickenDrawMeshes.push(_chickBodyMesh, _chickLegLMesh, _chickLegRMesh, _chickSparkleMesh);
  ...
  scene.add(_group);
  _clock = 0;
  _bDrawCount = _mDrawCount = _cDrawCount = 0;
  _loaded = true;
}
...
export function disposeForestPickups(scene) {
  ...
  _bombDrawMeshes.length = 0;
  _magnetDrawMeshes.length = 0;
  _chickenDrawMeshes.length = 0;
  _clock = 0;
  _bDrawCount = _mDrawCount = _cDrawCount = 0;
  _loaded = false;
}
